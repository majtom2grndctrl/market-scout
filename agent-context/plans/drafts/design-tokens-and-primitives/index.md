# Design Tokens and Layout Primitives

## Goal

Give `apps/web` a token-driven primitive layer — `Box`, `Text`, `Stack`, `Grid`, `Container` — so page and feature code composes layout from typed props instead of Tailwind class strings. A TypeScript token module becomes the single source for both the Tailwind `@theme` block and the primitives' prop types, so the style scale and the API cannot drift apart.

This is the foundation the seven views in `research/ui-view-catalog.md` get built on. Establishing it before product screens exist is the cheap moment; retrofitting it after is not.

## Scope

### In scope

- Token source modules under `apps/web/tokens/` — spacing, type size, font weight, leading, tracking, breakpoint names, container widths, grid track minimums.
- A codegen step emitting four artifacts from those tokens: the `@theme` CSS block, `@utility` blocks for composites with no theme-token equivalent, static Tailwind class maps, and prop union types.
- Five primitives with closed prop sets: `Box`, `Text`, `Stack`, `Grid` (with `GridItem`), `Container`.
- Responsive prop support via a per-breakpoint object form.
- Storybook stories for every primitive and a token-reference story.
- Rewriting `app/page.tsx` to use primitives, as the first consumer proving the API.

### Out of scope

- **Semantic color and radius tokens.** shadcn already owns these through the `@theme inline` block in `app/globals.css` and the `--primary`/`--background` custom properties. Replacing them breaks every future `shadcn add`. The token layer emits no `--color-*` or `--radius-*` property; it reads shadcn's color vars where primitives need color.
- **Replacing Tailwind's numeric spacing.** `--spacing: 0.25rem` stays. shadcn's `tailwind.css` computes against it in 34 places (shadcn 4.14.0); removing it breaks their utilities.
- **A JavaScript unit-test runner.** No test runner exists in `apps/web` today and adding one is its own decision. Verification for this plan is `tsc --noEmit`, `pnpm build`, `pnpm build-storybook`, the codegen drift check and probe, and the manual review gates listed below. See Open questions.
- **Named typography bundles.** No `variant="eyebrow"`. Recurring type combinations become composed components, per the design principle below.
- **`Inline`, `Spacer`, `Divider`, `Center`.** Follow-on once the prop conventions have been exercised by real screens.
- **Migrating `components/ui/button.tsx`.** shadcn components are vendored copies; rewriting them forfeits upstream updates. They are component files, so their Tailwind classes are already where classes belong.
- **Lint enforcement of the no-classes rule.** No ESLint config exists in the repo. The rule is carried by the primitives' types, which cover the primitives only — a raw `<div className="…">` in a page is still possible and is caught at review.
- **Arbitrary/escape values** (`p="17px"`). The closed prop set is the point.

## Acceptance criteria

- [ ] `pnpm tokens` regenerates every artifact from `apps/web/tokens/`; running it a second time produces no file diff.
- [ ] Appending a blank line to a generated file and running `pnpm tokens --check` exits non-zero.
- [ ] `pnpm build` runs `pnpm tokens --check` before `next build`.
- [ ] Every generated file opens with a generated-by header naming `pnpm tokens` — `// Code generated by pnpm tokens. DO NOT EDIT.` in TypeScript, `/* Code generated by pnpm tokens. DO NOT EDIT. */` in CSS — matching `agent-context/lib/developer-guide.md` §5.8.
- [ ] Editing a token value and rerunning codegen changes `tokens/generated/theme.css` and `tokens/generated/class-maps.ts`, and no `.css` file under `app/`.
- [ ] Adding step `1000` to the spacing scale makes `<Box p={1000} />` compile; removing it makes the same line fail `tsc --noEmit`.
- [ ] `tsc --noEmit` passes with every `@ts-expect-error` in the type-test files consumed, which fails if any closed prop stops rejecting `className`, `style`, an out-of-scale step, or a per-breakpoint object on a base-tier prop.
- [ ] The codegen probe confirms `p-4`, `p-2.5`, `h-8`, and `size-6` still compute against `--spacing`.
- [ ] The codegen probe fails when an emitted name is already defined in Tailwind's default theme and is not on the deliberate-override list.
- [ ] No file under `components/primitives/` imports `cn` or `tailwind-merge`.
- [ ] `app/page.tsx` contains no `className` attribute.
- [ ] `pnpm build`, `pnpm build-storybook`, and `tsc --noEmit` all exit zero.
- [ ] The emitted `tokens/generated/class-maps.ts` is at most 12 KB gzipped.
- [ ] `apps/web/package.json` exposes `tokens` and `typecheck` scripts, and its `build` script runs the drift check before `next build`.
- [ ] Every primitive has a story, and the plan's `index.md` carries a `## Prop-set gaps` section, empty or populated.

## Review gates

These are checked by a human in Storybook, not by a command. They are listed separately because a component that emits no classes at all still compiles and builds green — with no test runner, nothing mechanical observes rendered output. The deferred test-runner decision in Open questions would convert most of this list into acceptance criteria.

- [ ] `Box` renders the element named by `as`, defaults to `div`, and its background, radius, border, shadow, and overflow track the active theme without a per-theme prop.
- [ ] `Text` renders the element named by `as` with that element's default size and weight, and `<Text as="h2" size={900}>` renders an `h2` at the largest step.
- [ ] `Text` colors stay legible in both themes, and a `Text` carrying both `size` and `color` shows both classes on the element.
- [ ] `resolve-responsive.stories.tsx` shows a base class only where a `base` key is present, one prefixed class per other key, and nothing for absent keys.
- [ ] `Stack` lays out as a flex container, changes direction at breakpoints, and positions children via `align` and `justify`.
- [ ] `Grid` lays out as a grid, sets columns responsively, and `GridItem` spans columns and rows responsively.
- [ ] `Grid` with `columns="auto"` wraps items intrinsically at the `minItemWidth` track size.
- [ ] `Container` centers horizontally and constrains to its `maxWidth`.
- [ ] `Box` and `Stack` fill the viewport through `minHeight`.
- [ ] `id`, `role`, and `onClick` passed to any primitive appear on the rendered element.
- [ ] `app/page.tsx` renders the same layout, type hierarchy, and spacing as before; the heading's line height moves from 1.333 to 1.4, which is expected.
- [ ] `components/ui/button.tsx` renders identically to its pre-change appearance.
- [ ] Storybook stories render in Geist sans, not the browser default serif.
- [ ] Each primitive story shows no violations in Storybook's a11y panel.

## Token naming inventory

One name crosses three representations. Pin the transform once; every generated artifact follows it.

| Concept | TS token key | CSS custom property | Tailwind utility | Prop value |
|---|---|---|---|---|
| Spacing step | `spacing[400]` | `--spacing-400` | `p-400`, `gap-400`, `mx-400` | `p={400}` |
| Spacing zero | *(none — Tailwind built-in)* | *(none)* | `p-0` | `p={0}` |
| Type size | `text[400]` | `--text-400` (+ `--text-400--line-height`) | `text-400` | `size={400}` |
| Font weight | `weight[600]` | `--font-weight-600` | `font-600` | `weight={600}` |
| Leading | `leading.snug` | `--leading-snug` | `leading-snug` | `leading="snug"` |
| Tracking | `tracking.wide` | `--tracking-wide` | `tracking-wide` | `tracking="wide"` |
| Container width | `container.content` | `--container-content` | `max-w-content` | `maxWidth="content"` |
| Grid track minimum | `gridMin.md` | *(none — `@utility`)* | `grid-auto-md` | `minItemWidth="md"` |
| Breakpoint | `breakpoints.md` | *(TW default, not re-emitted)* | `md:` prefix | `{ md: ... }` key |

Rule: token keys are already in their final form, so the CSS property is `--<namespace>-<key>` verbatim. No camelCase-to-kebab conversion anywhere — that conversion is the drift risk this table exists to remove.

Spacing, type size, and weight take numbers. Everything else takes a name.

Every enumerated prop value and the exact class it emits lives in `prop-values.md`.

Verified against Tailwind 4.3.3: named `--spacing-*` and `--text-*` tokens generate `p-<name>` and `text-<name>` utilities and their responsive variants, coexisting with the numeric `--spacing` multiplier. A `--text-<n>--line-height` sub-property rides along on the same class, behind `var(--tw-leading, …)` so a `leading` utility overrides it cleanly. A `--font-weight-<n>` token generates `font-<n>`.

## Scale collisions

A token name that matches a Tailwind built-in either shadows it or is silently ignored, depending on namespace. Both failure modes are quiet.

| Namespace | Behavior | Consequence |
|---|---|---|
| `--spacing-*` | Named token wins over the numeric multiplier | `p-100` means the token, not 25rem. Intended. |
| `--container-*` | Loses to Tailwind's deprecated `--max-width-*` | `prose` is the single reserved name. Tailwind's deprecated block defines only `--max-width-prose: 65ch`, which wins over `--container-prose`. No other container name is shadowed. |
| `--leading-*`, `--tracking-*` | Named token overrides Tailwind's built-in of the same name | `tight`, `snug`, `normal`, `relaxed`, `wide`, `wider` all shadow defaults. Deliberate — the scale is ours. Tailwind's `loose`, `tighter`, and `widest` survive un-overridden. |

Verified against Tailwind 4.3.3 in both directions. Task 1 must compile a probe stylesheet and confirm each emitted token name resolves to its own value before the scale is considered settled.

## Spacing scale

Steps run 100–900, mirroring `font-weight`. All three numeric scales put their neutral value at 400: `font-weight: 400` is normal text, `--spacing-400` is the 1rem base, `--text-400` is body size.

| Step | Value | px |
|---|---|---|
| 0 | `0` | 0 |
| 100 | `0.25rem` | 4 |
| 200 | `0.5rem` | 8 |
| 300 | `0.75rem` | 12 |
| 400 | `1rem` | 16 |
| 500 | `1.5rem` | 24 |
| 600 | `2rem` | 32 |
| 700 | `3rem` | 48 |
| 800 | `4rem` | 64 |
| 900 | `6rem` | 96 |

The gaps are the point. A step between 400 and 500 lands at 450, a finer one at 425 or 475, and a sub-minimal step at 50 — all without renumbering anything downstream. A t-shirt scale forces a rename cascade or a `2xs`/`3xs` prefix pile-up at exactly that moment.

`0` is not a token. Tailwind ships `p-0` natively, so the class map points step `0` at the built-in utility and no `--spacing-0` property is emitted.

Verified against Tailwind 4.3.3: a defined `--spacing-100` shadows Tailwind's dynamic numeric utility, so `p-100` compiles to `var(--spacing-100)` (4px) rather than `calc(var(--spacing) * 100)` (25rem). Small numeric utilities (`p-4`, `p-2.5`, `h-8`, `size-6`) are unaffected, and neither shadcn's bundled CSS nor `apps/web` source uses any three-digit spacing utility, so nothing existing is shadowed.

The type size scale follows the same 100–900 shape. Its values, paired line heights, and per-element defaults live in `prop-values.md`.

## Tasks

### Task 1: Token modules and codegen

Build the token source and the generator every other task consumes.

- Write the generator at `apps/web/scripts/build-tokens.ts`, and its output to `apps/web/tokens/generated/theme.css`, `apps/web/tokens/generated/class-maps.ts`, and `apps/web/tokens/generated/types.ts`. Every Phase 2 task imports from those paths.
- Author tokens as `as const` TypeScript modules under `apps/web/tokens/`, one file per namespace. TypeScript is the source of truth, so the scale and the prop types come from one place.
- Emit the `@theme` block to a generated CSS file that `app/globals.css` imports. Keeping it separate from `globals.css` means codegen never has to parse or rewrite a hand-edited file.
- Emit an `@utility` block per grid track minimum. `repeat(auto-fit, minmax(…, 1fr))` has no theme-token equivalent, so it cannot ride the `@theme` path.
- Emit static class maps as nested objects keyed by breakpoint, then by prop value. Tailwind scans source as plain text and cannot resolve a runtime-built class name, so every class string must appear complete and literal in the generated file.
- Emit base-tier-only props as a flat value-to-class map, and type them as the bare token rather than `Responsive<T>`, so passing the object form to them fails at the call site.
- Emit prop union types derived from the same token keys.
- Emit a class map for every prop in `prop-values.md`. Phase 2 runs concurrently only if Task 1's output surface is closed.
- Export one class map per prop from `class-maps.ts`, named `<prop>Map` — `pMap`, `pxMap`, `gapMap`, `bgMap`, `radiusMap`, and so on. A per-prop export keeps each primitive's imports explicit.
- Shape responsive maps as `Record<'base' | Breakpoint, Record<Value, string>>` and base-tier maps as `Record<Value, string>`. `resolveResponsive` depends on that shape.
- Export the element-defaults map as `elementDefaults`, keyed by tag name, each entry carrying its size and weight class or nothing for the axes marked (no class).
- Emit the element-defaults map from `prop-values.md` as generated output. A ramp hand-copied into `text.tsx` would drift from the token module, which is the split this plan exists to prevent.
- Treat the padding and margin props in `prop-values.md` as the shared spacing props every primitive receives. The gap props are not shared — only `Stack` and `Grid` take them.
- Key the generated maps for the colliding prop names distinctly — `stackAlign`, `textAlign`, `stackWrap`, `textWrap`. Two props share each name across components, and a single map key would silently serve the wrong value set.
- Name every emitted custom property `--<namespace>-<key>` from the token key verbatim, with no case conversion. The key is already in final form.
- Emit each type size's paired line height as a companion `--text-<step>--line-height` property. Tailwind folds it into the same utility class behind `var(--tw-leading, …)`, which is what lets a `leading` prop override it.
- Emit the generated-by header as the first line of every artifact. §5.8 recognizes a generated file by that header alone.
- Export `splitStyleProps` from `shared.ts`, returning the style props and the DOM passthrough separately. Each primitive spreads the passthrough, so `id`, `role`, `aria-*`, and event handlers reach the element.
- Compile a probe stylesheet during the run and fail if any emitted name is absent from the deliberate-override list yet already defined in Tailwind's default theme. Checking only that a token resolves to its own value passes an accidental override, which is the failure mode.
- Assert in the same probe that `p-4`, `p-2.5`, `h-8`, and `size-6` still compute against `--spacing`. Shadowing runs both ways.
- Treat these emitted names as deliberate overrides of Tailwind built-ins: every `--spacing-<step>`, and `--leading-tight`, `--leading-snug`, `--leading-normal`, `--leading-relaxed`, `--tracking-tight`, `--tracking-normal`, `--tracking-wide`, `--tracking-wider`. Everything else must resolve to a name Tailwind does not already define.
- Never emit a `--container-prose`. Tailwind's deprecated `--max-width-prose: 65ch` wins over it, so the token would be silently ignored.
- Write the shared `Responsive<T>` type and a `resolveResponsive` helper by hand in `components/primitives/shared.ts`. Primitives call it rather than reimplementing breakpoint walking five times.
- Write the shared DOM-passthrough base prop type here too, not inside `Box`. All five primitives depend on it, and owning it in Task 1 is what keeps Phase 2 concurrent.
- Type the shared base as `React.HTMLAttributes<HTMLElement>` plus `ref`.
- Omit `color` from that type, because it collides with the prop name `Text` uses.
- Declare `className?: never`, because `Omit` alone still admits a spread object.
- Declare `style?: never`, for the same reason.
- Export `SpacingProps` and `SizingProps` types plus `spacingClasses()` and `sizingClasses()` helpers from `shared.ts`. Five primitives need the same fourteen spacing lookups, and five independent copies would drift.
- Treat `columns`, `colSpan`, `rowSpan`, and `lineClamp` as generator constants, not token modules. They are fixed numeric ranges with no design value behind them.
- Emit numeric and boolean prop unions as numeric and boolean literal types. Object keys stringify, so a union taken from map keys would yield `"400"` where the prop needs `400`.
- Run the generator with plain `node` — Node v26 strips TypeScript types natively, so no new dependency is needed.
- Set `allowImportingTsExtensions: true` in `apps/web/tsconfig.json`. Node's strip-only mode requires the `.ts` extension on relative imports, and without this flag `tsc --noEmit` fails those same imports with TS5097.
- Write relative imports in the generator and token modules with an explicit `.ts` extension, because Node's strip-only resolver does not infer it.
- Keep token modules and the generator inside Node's strip-only subset.
- Mark type-only imports with the inline `type` modifier, or they fail at runtime.
- Leave `apps/web/package.json` without a `type` field and accept Node's `MODULE_TYPELESS_PACKAGE_JSON` warning on each run. Setting `type: module` would change resolution for every `.js` file in the app.
- Support a `--check` mode that regenerates into memory and exits non-zero on any difference. This is what makes the drift check enforceable in `pnpm build`.
- Commit generated files. Builds must not require the generator to have been run first.
- Add the `@import` for the generated theme to `app/globals.css` immediately after its existing `@import` lines. CSS requires every import to precede other rules, and that file already has three.
- Add a `tokens` script to `apps/web/package.json` running the generator. The AC names a command that does not exist yet.
- Add a `typecheck` script running `tsc --noEmit`. The AC names a command that does not exist yet.
- Change the `build` script to run `pnpm tokens --check` before `next build`, so a stale generated file fails the build rather than shipping.
- Print the gzipped size of the emitted `class-maps.ts` and exit non-zero above 12 KB. A printed number that fails nothing is not a budget.
- Derive each numeric prop's TypeScript union from the token keys. Only defined steps are safe: an undefined three-digit spacing value silently falls back to Tailwind's multiplier, so `p-450` would compile to 112.5rem rather than failing. The union is what makes that unreachable from prop call sites.
- Generate responsive tiers only for the props in the table below. Full responsive coverage on every prop would add roughly 900 redundant entries against a 12 KB budget.

| Responsive | Base tier only |
|---|---|
| `p`, `px`, `py`, `m`, `mx`, `my` | `pt`, `pr`, `pb`, `pl`, `mt`, `mr`, `mb`, `ml` |
| `gap`, `gapX`, `gapY`, `direction`, `align` (stack), `justify` (stack) | `wrap` (stack), `bg`, `radius`, `border`, `shadow`, `overflow` |
| `columns`, `colSpan`, `rowSpan` | `weight`, `leading`, `tracking`, `color`, `family`, `transform`, `truncate`, `lineClamp` |
| `size`, `maxWidth`, `width`, `height`, `minHeight` | `minItemWidth`, `wrap` (text), `align` (text) |

- Create `apps/web/components/primitives/shared.type-test.tsx` asserting the bans that live in the shared type. Put one `// @ts-expect-error` above each invalid call — `className`, `style`, an out-of-scale step, and a per-breakpoint object on a base-tier prop. TypeScript reports an unused `@ts-expect-error` as TS2578, so `tsc --noEmit` fails when a ban stops working.
- Import the type-test file from nothing. It sits inside the tsconfig `include` glob, so `tsc` checks it, and Next never bundles it.
- Add `components/primitives/resolve-responsive.stories.tsx` rendering `resolveResponsive` output as text for a table of inputs, including a bare token, an object with `base`, and an object without. It is the only readable evidence of the breakpoint contract while there is no test runner.

Do not:
- Emit `--spacing` itself, or a `--spacing-0`, or any `--color-*` or `--radius-*` token.
- Use `--*: initial` to reset Tailwind's default theme.
- Hand-write any file the generator owns.
- Hand-edit `app/globals.css`'s existing `@theme inline` block.
- Use `enum`, `namespace`, or parameter properties anywhere the generator loads.

Tasks 2 through 6 each build one component. Every prop's allowed values, emitted classes, and responsive tier are in `prop-values.md`; that file is the complete prop surface, and no task invents a value.

### Task 2: Box

The spacing and surface primitive. Renders one element, no layout opinion of its own.

- Create `apps/web/components/primitives/box.tsx` as a named export.
- Import the shared types and helpers from `@/components/primitives/shared`, and the class maps from `@/tokens/generated/class-maps`.
- Spread the DOM passthrough from `splitStyleProps` onto the rendered element, so `id`, `role`, `aria-*`, and handlers reach it.
- Render exactly one element.
- Add a `box.type-test.tsx` beside the component with a `// @ts-expect-error` above one invalid call per closed prop. An unused `@ts-expect-error` is itself a type error, so this is what proves the prop set is closed.
- Constrain `as` to the element union in `prop-values.md`, defaulting to `div`. A fixed union avoids generic polymorphic-component typing, which is where this kind of component usually stalls.
- Build on the shared base prop type from Task 1.
- Draw `bg` and `radius` from shadcn's `--color-*` and `--radius-*` properties, so surfaces track the active theme.
- Draw `border`, `shadow`, and `overflow` from Tailwind's built-in scales. shadcn defines no `--shadow-*` property.
- Carry `width`, `height`, and `minHeight` over the enumerated sets in `prop-values.md`. Page shells need viewport height, and it is the one thing the current `app/page.tsx` cannot otherwise express.
- Emit a class for every set prop and do not deduplicate. Overlaps like `p` and `px` resolve by Tailwind's stylesheet order, exactly as raw utilities would.
- Join classes with `clsx`.

Do not:
- Declare a second base prop type. The `className` ban lives in Task 1's shared type.
- Import `cn` from `@/lib/utils`. It runs tailwind-merge, which does not know these scales and silently drops classes — `twMerge('text-400 text-muted-foreground')` returns only the color.
- Create or edit a barrel `index.ts`. Four other agents run concurrently and would collide on it.
- Import or render another primitive. It serializes this phase and makes class precedence depend on nesting order.

### Task 3: Text

Typography as orthogonal props. `as` names the element; `size` and `weight` name the look.

- Create `apps/web/components/primitives/text.tsx` as a named export.
- Import the shared types and helpers from `@/components/primitives/shared`, and the class maps from `@/tokens/generated/class-maps`.
- Spread the DOM passthrough from `splitStyleProps` onto the rendered element, so `id`, `role`, `aria-*`, and handlers reach it.
- Render exactly one element.
- Add a `text.type-test.tsx` beside the component with a `// @ts-expect-error` above one invalid call per closed prop. An unused `@ts-expect-error` is itself a type error, so this is what proves the prop set is closed.
- Constrain `as` to the text-bearing element union in `prop-values.md`, and default it to `p`.
- Apply the element's default size and weight from the ramp when neither prop is given, so `<Text as="h2">` is correct without further configuration.
- Let `size`, `weight`, `leading`, and `tracking` override those defaults independently. Decoupling visual level from document structure is the accessibility case for this component.
- Take `color` from shadcn's semantic foreground vars.
- Accept `family`, `transform`, `truncate`, and `lineClamp` over the value sets in `prop-values.md`. All four are base tier.
- Take `align` from `textAlignMap` and `wrap` from `textWrapMap`, never the `stack`-prefixed maps. Both names exist twice with different value sets, and the wrong map compiles cleanly while admitting values that do nothing.
- Emit nothing for an axis whose element default is marked (no class) in `prop-values.md`. `strong` sets weight only; `em` sets neither.
- Accept `htmlFor`, `dateTime`, and `cite` on every element rather than gating them on `as`. Gating needs a discriminated union per tag, which is the polymorphic typing this plan avoids.
- Read element defaults from the generated map, not from a table inside the component.
- Join classes with `clsx`.

Do not:
- Add a `variant` prop or any named type bundle. Recurring combinations become composed components.
- Take `color` from raw palette values. Only shadcn's semantic foreground vars survive a theme switch.
- Import `cn` from `@/lib/utils`. It runs tailwind-merge, which does not know these scales and silently drops classes — `twMerge('text-400 text-muted-foreground')` returns only the color.
- Create or edit a barrel `index.ts`. Four other agents run concurrently and would collide on it.
- Import or render another primitive. It serializes this phase and makes class precedence depend on nesting order.

### Task 4: Stack

One-dimensional flex layout. Supersedes the `HStack`/`VStack` pair.

- Create `apps/web/components/primitives/stack.tsx` as a named export.
- Import the shared types and helpers from `@/components/primitives/shared`, and the class maps from `@/tokens/generated/class-maps`.
- Spread the DOM passthrough from `splitStyleProps` onto the rendered element, so `id`, `role`, `aria-*`, and handlers reach it.
- Render exactly one element.
- Add a `stack.type-test.tsx` beside the component with a `// @ts-expect-error` above one invalid call per closed prop. An unused `@ts-expect-error` is itself a type error, so this is what proves the prop set is closed.
- Always emit the base class `flex`. Without it `direction`, `align`, and `justify` set properties on a block element and silently do nothing.
- Default `direction` to `col`. A Stack with no direction is a vertical stack.
- Make `direction` responsive, so one component covers what Chakra splits into two.
- Accept the shared spacing props alongside `align`, `justify`, and `wrap`.
- Take `align` from `stackAlignMap` and `wrap` from `stackWrapMap`, never the `text`-prefixed maps. Both names exist twice with different value sets.
- Accept `gap`, `gapX`, and `gapY` over the spacing scale. The shared spacing props cover outer spacing only.
- Constrain `as` to the element union in `prop-values.md`, defaulting to `div`. Page shells are flex containers that need to render as `main` or `section`.
- Carry `width`, `height`, and `minHeight` over the value sets in `prop-values.md`, since the page shell is usually a `Stack`.
- Join classes with `clsx`.

Do not:
- Import `cn` from `@/lib/utils`. It runs tailwind-merge, which does not know these scales and silently drops classes — `twMerge('text-400 text-muted-foreground')` returns only the color.
- Create or edit a barrel `index.ts`. Four other agents run concurrently and would collide on it.
- Import or render another primitive. It serializes this phase and makes class precedence depend on nesting order.

### Task 5: Grid and GridItem

Two-dimensional layout for scorecards and dashboards.

- Create `apps/web/components/primitives/grid.tsx` as a named export.
- Import the shared types and helpers from `@/components/primitives/shared`, and the class maps from `@/tokens/generated/class-maps`.
- Spread the DOM passthrough from `splitStyleProps` onto the rendered element, so `id`, `role`, `aria-*`, and handlers reach it.
- Render exactly one element.
- Add a `grid.type-test.tsx` beside the component with a `// @ts-expect-error` above one invalid call per closed prop. An unused `@ts-expect-error` is itself a type error, so this is what proves the prop set is closed.
- Always emit the base class `grid`. `grid-cols-3` sets the template but not the display mode.
- Make `columns`, `GridItem`'s `colSpan`, and `GridItem`'s `rowSpan` responsive; a fixed column count is unusable on the composite views.
- Support `columns="auto"` paired with a `minItemWidth` token, applying the `@utility` class Task 1 generates. Card grids need intrinsic wrapping, and it cannot be expressed as a column count.
- Exclude `auto` from the responsive object form of `columns`; it is valid only as a bare value. `minItemWidth` is base tier, so a per-breakpoint auto-fit would need a utility per breakpoint per width.
- Require `minItemWidth` whenever `columns` is `auto`, and ignore it otherwise. `columns="auto"` alone would emit a display mode and no template.
- Accept `gap`, `gapX`, and `gapY` over the spacing scale. A grid without gap control cannot lay out the scorecard views.
- Give `GridItem` the same `as` union and the shared spacing props as the other primitives, defaulting to `div`.
- Export `Grid` and `GridItem` from the same `apps/web/components/primitives/grid.tsx` file; they are one layout contract.
- Join classes with `clsx`.

Do not:
- Import `cn` from `@/lib/utils`. It runs tailwind-merge, which does not know these scales and silently drops classes — `twMerge('text-400 text-muted-foreground')` returns only the color.
- Create or edit a barrel `index.ts`. Four other agents run concurrently and would collide on it.
- Import or render another primitive. It serializes this phase and makes class precedence depend on nesting order.

### Task 6: Container

Page-width shell.

- Create `apps/web/components/primitives/container.tsx` as a named export.
- Import the shared types and helpers from `@/components/primitives/shared`, and the class maps from `@/tokens/generated/class-maps`.
- Spread the DOM passthrough from `splitStyleProps` onto the rendered element, so `id`, `role`, `aria-*`, and handlers reach it.
- Render exactly one element.
- Add a `container.type-test.tsx` beside the component with a `// @ts-expect-error` above one invalid call per closed prop. An unused `@ts-expect-error` is itself a type error, so this is what proves the prop set is closed.
- Always emit `mx-auto`. That is the centering, and it is the one literal class a primitive emits outside the generated maps.
- Center horizontally by default and constrain to a named `maxWidth` token.
- Accept the shared padding and margin props; `px` is the responsive page margin.
- Constrain `maxWidth` to the values in `prop-values.md`; it is a responsive prop.
- Constrain `as` to the element union in `prop-values.md`, defaulting to `div`.
- Join classes with `clsx`.

Do not:
- Import `cn` from `@/lib/utils`. It runs tailwind-merge, which does not know these scales and silently drops classes — `twMerge('text-400 text-muted-foreground')` returns only the color.
- Create or edit a barrel `index.ts`. Four other agents run concurrently and would collide on it.
- Import or render another primitive. It serializes this phase and makes class precedence depend on nesting order.

### Task 7: Stories and first consumer

- Add one story per primitive — Box, Text, Stack, Grid, GridItem, Container — plus one token-reference story rendering the spacing scale, the type scale, and every element default.
- Give each primitive an all-props matrix story, so one screen covers its whole surface for review.
- Put every story under `components/primitives/`, including the token-reference story. The Storybook glob in `.storybook/main.ts` only scans `../components/**`, so a story placed beside its tokens would silently not load.
- Import `Geist` from `next/font/google` in `.storybook/preview.ts` and add a decorator applying its `variable` class to a wrapper inside `#storybook-root`. `globals.css` defines `--font-sans: var(--font-sans)` self-referentially, and only `app/layout.tsx` breaks that cycle — Storybook otherwise renders every story in the browser default serif.
- Hand off to a human for a11y review in Storybook's panel, and note the handoff in the completion report. The addon reports only in the UI, so this is a review gate rather than something the task can close.
- Run `pnpm build-storybook` and require a zero exit. It is the only check that every story actually compiles.
- Rewrite `app/page.tsx` as a `Stack` with `as="main"`, `minHeight="svh"`, `direction="col"`, `align="center"`, `justify="center"`, `gap={400}`, wrapping the heading and the existing `Button`.
- Render the heading as `<Text as="h1" size={600} weight={600}>`. The current markup is `text-2xl font-semibold`; `as="h1"` alone would pick up the element default of 900/700 and trip the size from 1.5rem to 3.5rem.
- Leave `Button` as it is. shadcn components get composed into primitive layout, not replaced.
- Append a `## Prop-set gaps` section to this plan's own `index.md`, wherever it currently sits under `agent-context/plans/`, listing everything `app/page.tsx` could not express.

Do not:
- Add `className` to a primitive to unblock the rewrite.
- Hand-edit a generated map to unblock the rewrite. Add the value to the token module and rerun `pnpm tokens`.

## Sequencing

**Phase 1 (sequential):** Task 1 — every other task imports its generated maps and types.
**Phase 2 (concurrent):** Tasks 2–6 — one file each, no composition between them; all five share only Task 1's helpers.
**Phase 3 (sequential):** Task 7 — consumes all five primitives.

Primitives must not compose each other. `Stack` rendering a `Box` internally would serialize Phase 2 and make class precedence depend on nesting order.

Each primitive is a named export from its own file, imported as `@/components/primitives/box`. No barrel file — five concurrent agents would collide on it.

Primitives join classes with `clsx` and never with `cn`. `cn` runs `tailwind-merge`, which does not know the custom scales and silently discards classes: verified against the installed tailwind-merge 3.6.0, `twMerge("text-400 text-muted-foreground")` returns `"text-muted-foreground"`, dropping the size. It reads any `text-*` class it cannot place in a known font-size scale as a color, and a size and a color cannot coexist in its model. `font-*` fails the same way: `twMerge("font-600 font-sans")` returns `"font-sans"`, dropping the weight. Both of `Text`'s `text-*` props and both of its `font-*` props are at risk, so the problem is any prefix carrying two meanings, not one prefix.

Overlapping props like `p` and `px` resolve by Tailwind's own stylesheet order, exactly as the equivalent raw utilities would. That is the precedence a Tailwind user already expects, so merging adds nothing. `components/ui/` keeps using `cn`; this rule is scoped to `components/primitives/`.

## Rough sketch

```
apps/web/
  tokens/
    spacing.ts            # canonical
    typography.ts         # canonical — size, weight, leading, tracking, element defaults
    layout.ts             # canonical — containers, grid track minimums, sizing
    breakpoints.ts        # canonical — names only; values stay Tailwind defaults
    generated/
      theme.css           # @theme + @utility blocks, imported by app/globals.css
      class-maps.ts       # static Tailwind class strings + element defaults
      types.ts            # Space, TextSize, Weight, Breakpoint unions
  components/primitives/
    shared.ts             # Responsive<T>, resolveResponsive(), base prop type — hand-written
    box.tsx  text.tsx  stack.tsx  grid.tsx  container.tsx
  scripts/build-tokens.ts
```

```tsx
// Proposed design
type Responsive<T> = T | ({ base?: T } & Partial<Record<Breakpoint, T>>)

<Stack as="main" minHeight="svh" direction={{ base: 'col', md: 'row' }} gap={500} align="center">
  <Text as="p" size={100} weight={600} tracking="wider" transform="uppercase" color="muted">
    Demand trend
  </Text>
  <Text as="h2" size={900}>Stripe</Text>
</Stack>
```

`resolveResponsive` walks the breakpoint keys in order and indexes the generated map per tier, so `direction={{ base: 'col', md: 'row' }}` yields `flex-col md:flex-row`. `shared.ts` imports `Breakpoint` from the generated types rather than restating the list, so the breakpoint set has one definition.

The eyebrow above is deliberately verbose. Five props is the signal that an `<Eyebrow>` component should exist — see below.

## Design principle: friction is the feedback

The closed prop set is not a constraint to work around. These primitives are atomic, so their props should cover ordinary layout completely. A page that cannot express itself in them signals one of two things, and both are worth hearing:

- **A one-off need** — build a real component for it. That is the intended outcome, and it keeps the exception named and reviewable instead of scattered as inline classes.
- **A recurring need** — the system is wrong. Repeated reaching for the same missing value means the spacing scale has a gap, the color tokens are too rigid, or a prop is missing. Fix the system, not the call site.

An escape hatch would convert both signals into silence. That is the reason there isn't one.

This is also why `Text` has no `variant` prop. A named bundle like `eyebrow` is a component wearing a prop's clothing: it hides that a recurring pattern exists, and it grows one entry at a time until the variant list is the design system. Spelling the combination out at the call site makes the repetition visible, and the second time it appears it becomes a component with a name.

Task 7 records what `app/page.tsx` could not express. That list is the first real measurement of whether the scale is right.

## Open questions

- **Test runner.** Planned as the immediate follow-up, not a someday item. The primitives are pure value-to-class functions and `resolveResponsive` is the one piece with real branching, so the highest-value tests are cheap. Vitest is the likely fit, but it is a new dependency and the first JavaScript testing convention in a repo whose `testing-guide.md` is Go-only — which is why it is its own decision rather than a bullet inside this plan. The a11y criterion is a manual Storybook check until it lands.
- **Element defaults as a coupling risk.** `<Text as="h2">` picking up size 800 makes the common case terse, but it does mean the element still carries visual meaning by default. The override is one prop away and the AC covers it, so this is a watch item rather than a flaw. Revisit if pages start setting `size` on most headings — that would mean the ramp is wrong.
- **`agent-context/lib/` capture at promotion.** Three durable facts belong in the library, not in this plan: the token pipeline's direction (TypeScript source, CSS generated, never the reverse) and the closed-prop-set principle above, both in `project.md`; and the generated-file rows for `pnpm tokens` in `developer-guide.md` §5.8.
