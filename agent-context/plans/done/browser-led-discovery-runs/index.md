...
- A `dedup_candidates` read-only MCP tool: batch-classify candidates (each `name`, optional `ats`/`board_token`) as `new` / `duplicate` / `stale`, with per-result evidence fields explaining the match path, using the `watchlist.md` §Dedup rules.
...
- [x] `dedup_candidates` accepts a bounded batch of candidates (each `name` required; `ats`, `board_token` optional) plus a separate top-level, optional `recency_days` (default 30) applied to the whole batch, and returns one verdict per candidate without opening a browser, making HTTP requests, or writing to the DB. It binds the read-only pool.
- [x] A candidate with a supplied `(ats, board_token)` matching a tracked company with a snapshot inside the recency window returns `duplicate`; a match with no recent snapshot returns `stale`, with the matched company surfaced.
- [x] A candidate matched only by normalized name (punctuation and whitespace stripped, lowercased, equality — substring is not a match) returns `stale` with the matched company surfaced, never a silent `duplicate`.
- [x] A candidate with no `(ats, board_token)` match and no normalized-name match returns `new`.
- [x] Each result includes `match_kind`, `reason`, `match_count`, and `matches` fields so clients can distinguish token duplicates, token-stale matches, name-only review cases, no-match results, ambiguity, and per-candidate validation failures without inferring from `matched.has_recent_snapshot`.
- [x] `recency_days` defaults to 30 when omitted. Name normalization has one shared definition; the tool does not carry a second copy of the rule.
- [x] An invalid candidate (empty `name`) does not fail the batch. Its `results` element returns `verdict = invalid` and a per-element `error` (`actionError`, `path` = `candidates[i].name` following `save_enrichment`'s `canonical_roles[0].slug` precedent, `code` a stable snake_case code, `message` a hint); sibling candidates in the same call still return their verdicts. `ok=false` with empty `results` is reserved for call-level failure (e.g. DB error), not per-candidate validation. Never an MCP transport error.
- [x] `watchlist.md` documents the browser-led loop: `dedup_candidates` → browser investigation of survivors → `detect_ats` → `add_company`, and distinguishes the three sourcing paths (batch sidecar, one-off live onboard, browser-led preflight).
- [x] From `apps/tools`: `go test ./cmd/mcp -count=1`, `go test ./...`, `go build ./...`, and `go vet ./...` pass. `staticcheck ./...` could not be run because `staticcheck` is not installed in this environment.

## Tasks

### Task 1: Dedup preflight tool

Add a `dedup_candidates` read-only MCP tool bound to `pools.readOnly`, following `enrichment_preview`'s tool conventions (`mcp.NewTool` + `s.AddTool`, request DTO via `BindArguments`, `ok=false` envelope for tool-level failures, a `pool*`-backed seam wired by `dedupCandidatesHandler(pool)` into a `dedupCandidatesHandlerWithDeps` body for tests) — `enrichment_preview` is the only existing tool with this ok-envelope-plus-seam shape; `detect_ats` is DB-free and out of scope as a model here. `"dedup_candidates"` is the MCP tool string only — it stays snake_case; the Go identifiers are camelCase. It takes a batch of candidates and a top-level optional `recency_days *int` (default 30, following `previewRequest.Count *int`), returns a verdict per candidate, plus `match_kind` and `reason` evidence fields. Empty `name` is validated per candidate after a successful `BindArguments`: the offending element returns `verdict = invalid` with a per-element `error`; sibling candidates still get verdicts. Partial failure, not whole-batch reject — one bad name in a scraped list never discards the rest, which matters for unattended runs. This diverges from `enrichment_preview`, which rejects the whole call on bad input. `ok=false` is reserved for call-level failure; a genuinely undecodable tool call still returns an MCP transport error.
...
Dedup match kind (JSON `match_kind`, tool-only, not persisted): `token`, `name_only`, `none`.

Dedup reason (JSON `reason`, tool-only, not persisted): stable snake_case strings explaining the verdict path. Reasons distinguish token matches with recent snapshots, token matches without recent snapshots, name-only matches, no match, and missing required name.

Dedup ambiguity fields (JSON `match_count` and `matches`, tool-only, not persisted): name-only matches keep every normalized-name collision in deterministic company-id order. `matched` remains the primary row for backward-compatible clients.
...
| Matched company | `Matched` | `"matched"` | output; primary `id`/`name`/`ats`/`board_token`/`industry`/`careers_page_url`/`has_recent_snapshot` when matched, else null — `name` from the added `FindCompanyDedupStatus` column on the token branch (from the name-match query directly on the name branch); `ats`/`board_token` may echo the input |
| Match count | `MatchCount` | `"match_count"` | output; number of rows in `matches` |
| All matches | `Matches` | `"matches"` | output; all deterministic matched rows, empty when none |
...
1. **Normalization location** — resolved during implementation: SQL owns normalization, so the input array passes raw names and the rule has one DB implementation.
2. **Batch size bound** — resolved during implementation: cap `dedup_candidates` at 200 candidates per call, matching the MCP schema and handler validation.
