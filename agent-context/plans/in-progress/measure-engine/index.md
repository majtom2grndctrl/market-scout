...
- `week` grouping buckets by first-seen (posting entry week) for every measure, so a posting seen across many runs resolves to one week and needs no per-run disambiguation. `rate` is the canonical week-grouped measure; a week-grouped `count`/`delta` is the same inflow bucketing applied to that measure.
...
- Define `MeasureResult` and its row type once, shared by all measures — Tasks 3, 4, and 5 all build against this exact contract, so pin it here:
  ...
  interface MeasureRow {
    ...
    gap?: boolean;                           // rate only: week with no successful run in scope; value is 0, the flag is the signal
  }
