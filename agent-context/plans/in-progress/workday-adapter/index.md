...
- [ ] A Workday company's fetch produces a `job_postings` row with `source_id` and `source_url`
      populated, and a `posting_snapshots` row with `title`, `location_text`, `job_url`, and
      `posted_at` populated. `description_text` is NULL on the snapshot — accepted for v1.
- [ ] Companies with `ats='workday'` but an unparseable board_token produce a wrapped error (with `workday:` prefix) and skip that company (no crash, other companies proceed).
...
- [ ] Pagination: a board with more jobs than the page limit is fully fetched across multiple
      requests.
- [ ] The sanity ceiling on pagination triggers a wrapped error (not an infinite loop) for a
...
Same User-Agent header (`market-scout/0.1 ...`). Mirrors `httpFetch` verbatim on response
handling: no retries, 32 MiB response cap, 4 KiB error-snippet cap, 2xx-only status
validation. Caller marshals `body`.
...
A board_token that does not contain `/` or yields an empty host/site/tenant is a wrapped
error; the fetcher skips that company and logs a `[workday] invalid board_token` warn.
Trim leading/trailing whitespace before parsing. board_tokens that contain `://` or
start/end with `/` are rejected as invalid.
...
overridden. When `baseURL` is set, the request URL is `{baseURL}/wday/cxs/{tenant}/{site}/jobs`.
The constructed fallback `SourceURL` continues to use the parsed `host` from `boardToken` —
tests should populate `externalUrl` in fixture rows to avoid asserting against real Workday
hostnames.
...
Per-page body: `{Limit: 20, Offset: pageOffset, SearchText: "", AppliedFacets: map[string]any{}}`. Marshal with `json.Marshal` and pass the bytes to `httpPost`.

**SourceID:** `externalPath` (unique per posting, stable across fetches). Empty `externalPath` returns `workday: job at index N for {boardToken} has missing externalPath` (mirrors lever.go).

**SourceURL:** `externalUrl` when non-empty; otherwise constructed fallback `https://{host}/en-US/{site}/{externalPath}`. Required; an empty resolution is a wrapped error.

**JobURL:** `ptrIfNonEmpty(resolvedURL)` — the same resolved string as `SourceURL`, wrapped via the helper used by lever.go.
...
Exceeding 1000 pages returns `fmt.Errorf("workday: pagination exceeded sanity ceiling for %s", boardToken)` (mirrors lever.go).
...
slice. Do not split — the delimiter is not stable across boards. Also set `LocationText` to a
pointer to the wrapped string (mirrors lever.go's `posting.LocationText = &first`).
...
- `TestWorkdayAdapter_StopsWhenOffsetExceedsTotal` — `total` is 25, page 1 returns 20 jobs (offset advances to 20), page 2 returns 5 jobs; loop terminates because `offset + len(rawJobs)` = 20 + 5 = 25 >= total.
- `TestWorkdayAdapter_SourceURLFallback` — a job with empty `externalUrl`; verify `SourceURL` is `https://{host}/en-US/{site}/{externalPath}`.
...
- `TestWorkdayAdapter_MissingExternalPath` — a job entry with empty `externalPath`; expect
  wrapped error naming the index.
...
| Posted date | `wdJob.PostedOn` | `"postedOn"` | YYYY-MM-DD when present → PostedAt, SourceFirstPublishedAt. Relative strings (e.g. `"Posted N Days Ago"`) tolerated: warn + leave fields nil. |
