# Project Overview

> **Read this when:** starting any session or onboarding to this codebase. Covers purpose, audience, and settled architectural decisions.
> **Key invariant:** decisions here are load-bearing — don't relitigate without explicit discussion.
> **Related:** `index.md` (decision table), `style-guide.md` (writing conventions)

---

## What it is

Local-first market intelligence tool. Tracks job postings across companies of interest. Surfaces role trends, hiring patterns, and market dynamics over time. "Personal Crunchbase for hiring data, scoped to a niche."

## Audience

Initially the author. Open-source so other AI-product builders (designers, PMs, hybrid roles) can fork and run their own instance with their own company list.

The person reads the market against their own experience. A résumé becomes a profile of past titles and skills. The profile suggests roles — including ones the person has never heard named. The person pins the roles worth watching, and a dashboard follows them.

## Why it exists

The point is AI-native interaction design. Every analysis on screen is a composition over a small vocabulary of analytical primitives — measure, grouping, filter, encoding. The primary surface starts from a person, not a prompt: an empty prompt box asks someone to already know the question. A dashboard composes analyses for the pinned roles, choosing which to show from what is changing in the data. Chat comes later, for questions the dashboard did not anticipate. The designed artifact is that vocabulary and how selection — by rule, by model, or by the person — reaches combinations that are novel, sound, and honest about coverage. Not the transport under it, and not a fixed catalog of screens. See [`research/composition-grammar.md`](../../research/composition-grammar.md).

Two goals stand under it and keep it honest:

1. Credible backend skills as a generalist. The backend is built to be learned from.
2. Real market intel from day one, compounding into trend analysis over weeks and months. The data has to be true for the interaction to mean anything — an agent composing over noise is a toy.

Build-versus-buy splits here. Backend layers are built. Frontend transport is bought, so attention stays on the interaction. The interaction vocabulary itself is built, never bought — it is the artifact.

Not a product to sell. A personal tool that doubles as a portfolio piece and learning vehicle.

## Repo layout

`apps/` houses deployable units. The Go module (binaries and shared packages) lives at `apps/tools/`; the module path is `github.com/majtom2grndctrl/market-scout/apps/tools`. The Next.js app lives at `apps/web/` as a sibling. Shared infra (`docker-compose.yml`, root `.env.local`) stays at repo root and serves both.

`apps/web/app/prototypes/` is the second low-attention pocket, and the only one that is running code. Sketches live there against real tokens and real data so a page can be seen before it is specified. Agents read it only when directed, for the same reason they skip `research/`: an abandoned sketch read as precedent asserts a decision nobody made. See [`web-guide.md`](./web-guide.md).

`research/` sits at repo root as a deliberate low-attention pocket — quasi-documentation that agents read only when explicitly directed. It is not under `agent-context/` (read by default) and not under `apps/` (deployable code). Tools reference its contents via CLI arg, never a hardcoded path.

## Settled architecture

| Layer | Choice | Notes |
|---|---|---|
| Fetcher | Go binary, `apps/tools/cmd/fetcher` | Concurrent HTTP from ATS APIs, cron-scheduled |
| Classifier | Codex `batch-enrich` skill over project MCP; unattended `apps/tools/cmd/classify` | Two paths, both writing append-only through the constrained `mcp.save_enrichment` action. The human-operated skill delegates bounded workers and mints taxonomy. The unattended classifier picks only existing terms, one instrument per dimension. It replaces `apps/tools/cmd/batch-enrich`, a legacy runner that writes directly, outside that boundary; see below. |
| Enrichment selection | Recency-first, capped per company | Both classifier paths share one selection core, so they cannot drift. Newest description-bearing postings go first, and a wave cycles across companies under a per-company ceiling before it fills on recency. Draining oldest-first kept the classified set months behind the market; sampling the corpus proportionally would make title analysis a report on three companies. Under-sampling the giants is the point. |
| Database | Postgres in Docker | Option to point at Supabase later |
| DB client | `sqlc` + `database/sql` + `pgx/v5/stdlib` | Write SQL, get generated type-safe Go. No ORM. Standard `database/sql` target (no `sql_package` override); generated models use `sql.NullString`/`sql.NullTime`, translated to `*string`/`*time.Time` at the DB boundary. |
| Vector search | pgvector extension | Enabled from day one. Similarity queries in raw SQL. |
| Storage model | Append-only snapshots | Every fetch writes timestamped rows. Never upsert. Load-bearing for trend analysis. |
| App layer | Next.js Server Components → Postgres direct | No separate Go API server. Reads run in Server Components on the `postgres` npm package over the read-only DSN. Server Actions arrive with the first write path. Route handlers are reserved for streaming — see below. |
| Read model | SQL views in numbered migrations | Derived "current" state — open postings, latest snapshot, latest classification — defined once in SQL rather than per consumer. |
| Analysis surface | Composition grammar over primitives | Every analysis is one measure × grouping × filter, with cohort/sort/limit modifiers, rendered by an encoding. A detector, a model, or the person composes it; the backend computes every aggregate, and no model emits a number. Deep link is the serialized composition. See [`research/composition-grammar.md`](../../research/composition-grammar.md). |
| Profile and pins | One profile per install; pins are canonical roles | Résumé extraction is reviewed before save. Title heads label a pinned role; they are never the pin. See below. |
| Dashboard | Widget = one composition + the reason it is shown | Deterministic detectors propose, a ranker chooses. Models narrate first, select later, compute never. See below. |
| Model access | OpenRouter, runtime — interactive surfaces and the unattended classifier | First runtime model-API dependency. Serves résumé extraction, widget narration, chat, and the unattended classifier's model judgments. Interactive bulk classification uses the direct-MCP Codex skill. One key across model families makes composition quality measurable per model. |
| Charts | Hand-rendered SVG; `d3-scale`, `d3-shape`, `d3-array` | No chart component library. d3 supplies scales and path geometry; every mark is app-owned, so charts inherit design tokens instead of carrying a second theme. Render target is any shape the grammar produces, not a fixed set of forms. Survey: [`research/agent-ui-landscape.md`](../../research/agent-ui-landscape.md) §Charts. |
| UI stack | App Router, TypeScript, Tailwind v4, shadcn/ui on Base UI | Components are copied into the repo, not imported. `shadcn add` serves the Base UI variant, not Radix — set in `components.json` (`style: base-nova`, `tsx: true`). Schibsted Grotesk for body, Funnel Display for headings, Lucide icons, neutral base color. |
| Layout and type | Tailwind's own scales | Type and spacing scales are Tailwind's defaults; shadcn owns color and radius. Two additions are ours: font-family tokens in `app/globals.css`, container widths in `app/tokens.css`. See [`web-guide.md`](./web-guide.md). |
| Future | `init` CLI | Scaffolds new users' setups. Not in scope yet. |

Our own first-observed timestamp on a job-posting record is the load-bearing repost-detection signal. ATS-reported "first published" and "created at" timestamps refresh on repost on at least some boards, so they cannot be trusted as the primary signal. The append-only snapshot model combined with an immutable first-observed timestamp — set once on initial upsert, never overwritten — is what makes repost detection durable. Source-reported timestamps are captured on every snapshot as a secondary change-detection signal, not as the repost anchor.

Every fetch is recorded as a fetch-run row, one per company per invocation, capturing the outcome and timing of that attempt. New snapshots link back to the run that produced them; snapshots written before the fetch-run migration carry a NULL run reference. This separation is load-bearing for trend queries: a posting's absence from a fetch only counts as "removed" when the run for that company succeeded. A failed run, or a company we didn't fetch in a given window, is distinguishable from a posting that genuinely disappeared from the board. Without the run record, a network blip looks identical to a wave of closed roles.

Nothing in the schema records whether a posting is currently open. It is derived: a posting is open when it appeared in the most recent **successful** fetch run for its company. Absence from a failed or in-progress run means nothing, which is what makes the fetch-run record above load-bearing rather than incidental. That derivation lives in SQL views, not in each consumer, because both the Next.js app and the agent's MCP `query` tool ask the same question and a second copy would drift. Latest snapshot and latest classification are derived the same way, per posting, from append-only history.

Derived state that reads a posting's current snapshot resolves it within that posting's latest successful fetch run, not by most-recent timestamp alone. Without the run predicate, a snapshot written by a later failed run can supply the current row — the same failure the fetch-run record exists to prevent, and one that stays invisible under fixture tests.

Posting count and requisition count are different numbers, and both are honest. A board may list one requisition once per location, so a company's posting count can overstate how many jobs it is filling. The read model derives requisition identity alongside the posting and the analysis surface reports both, rather than collapsing either. Collapsing at fetch would discard rows the append-only model exists to keep; reporting postings alone would overstate hiring wherever a board fans out. Where a platform exposes no requisition identifier, each posting is its own requisition, and the derived state records which of the two produced the key so a fallback is never mistaken for a platform fact.

A measure that cannot supply a number omits it rather than emitting zero. Zero is a real value in every measure the grammar offers, so an unavailable number has to be absent to stay distinguishable from a measured one.

Geography enters the analysis surface as a curated `market` dimension, never raw location text. Posting location strings are noisy — concatenated, suburb-fragmented, region-or-remote mixed — so the grammar excluded raw geography by design. A market dimension makes location reliable by curation: a seed dictionary maps observed strings to discrete markets (metro hubs, macro-regions, and remote tiers, tagged by grain), derived once in the read model alongside the other taxonomy terms and exposed as a first-class grouping and filter. A location matching no seeded market resolves to an explicit `unmapped` value, never silent absence, so a market grouping cannot imply coverage it lacks. Unlike the classified dimensions, market needs no coverage denominator: location is present on nearly every snapshot, so its only gap is dictionary breadth — which the visible `unmapped` value already carries.

The web app reads Postgres directly from Server Components. Next.js's server layer is the boundary; a route handler would add a second serialization hop inside the same process with no consumer on the other side. The agent does not read through the web app — it has the MCP server, whose read-only and action roles enforce a tighter privilege boundary than an HTTP API could.

One exception, stated as a test rather than a carve-out. A route handler earns its place when the client consumes the response incrementally over the life of one request — the chat surface, where tokens and tool results arrive across seconds. A Server Component resolves once and renders; it cannot hold a connection open. Request-response data fetching stays in Server Components no matter how convenient a route handler looks.

The interactive surfaces — résumé extraction, widget narration, chat — are the project's first runtime model-API dependency, reached through OpenRouter. A deliberate split, not a drift. Interactive batch classification uses the project-local Codex skill and constrained MCP action. The unattended classifier is the one bulk path that calls OpenRouter: it asks a decision model for typed choices with probabilities, never free text, at a per-posting cost small enough to run unattended. Interactive calls are low-volume, per-person, and the research subject itself — so they earn a runtime API, and one key across model families is what makes composition quality measurable per model. Across both, the model selects and combines primitives; it never computes an aggregate or emits a number. The backend computes; the model narrates.

When the first web-app write lands (user preferences, saved searches), it goes through Server Actions and plain table grants on a dedicated role. Not `SECURITY DEFINER` functions — the `mcp` schema uses those because the agent writes to core tables and the write *shape* needed constraining, whereas app-owned tables have one writer and no provenance requirement. Persisted query criteria are structured filter terms, never SQL text: stored SQL is an arbitrary-execution path and it rots silently when the views beneath it change.

One profile per install. A résumé becomes past titles and skills matched to taxonomy terms. Extraction is agent-asserted, so the person reviews every match before it is saved, and an item matching no term stays visible as unmatched rather than dropped. The résumé stays on the machine except for the extraction call itself, which goes to a third-party model provider — onboarding says so. A profile records what a person claims, not how good they are at it. Every score downstream is coverage of stated requirements, never hireability.

A person pins canonical roles, never title heads. Every insight worth showing hangs off the role: the skills it requires, the roles adjacent to it by shared skills, the companies hiring for it. A title head links to none of those, and the market spells one job many ways — a trend split across the spellings is a set of thin lines. Titles stay the vocabulary a person reads. A pinned role is labelled by the title heads its postings use, and an unfamiliar head under a role the person already does is the discovery the surface exists for. Taxonomy repair merges and retires roles, so a pin must follow its role through repair and never silently empty. Each install grows its own taxonomy from its own company list; no role is assumed to exist.

A dashboard widget is one composition plus the reason it is on screen. Selection starts deterministic. Detectors read measure-engine output for a pinned role — a shift against its own baseline, a new title head, a rising skill the person lacks — and propose scored candidates; a ranker chooses. A detector skips any window with thin classified coverage or a failed run, and says so, because a coverage cliff reads as a market collapse. Models enter second and still never compute: first narrating a widget from values the backend supplied, then choosing among detector candidates. Detector output doubles as the eval fixture for the model version.

Design tokens are hand-written CSS in a Tailwind `@theme` block, not a generated artifact. Tailwind already delivers tokens into utility classes, so a TypeScript-source-plus-codegen pipeline would only exist to keep prop types in sync with CSS — a problem the app does not have. Styling runs in two layers. Tailwind's scales are primitive tokens naming a size; custom utilities in `globals.css` are semantic tokens naming a purpose, composed from those primitives. A semantic name earns its place even when it duplicates a primitive's value, because intent survives a value change and a size name does not. Naming is constrained, though: shadcn's `cn()` runs tailwind-merge at every call site, and tailwind-merge drops a name it misreads. See [`web-guide.md`](./web-guide.md).

Classification runs on its own cadence. The interactive Codex workflow selects a bounded cohort through MCP, strips per-company boilerplate, and dispatches model workers in waves. Workers validate and save one structured candidate through the action-role-only `mcp.save_enrichment` boundary; a transaction-scoped database advisory lock serializes those writes so cross-table taxonomy ownership remains consistent even when workers are concurrent. The coordinator reconciles persisted classifications between waves and alone maintains the append-only failure history and report. The unattended classifier, `cmd/classify`, replaces the Go runner, `cmd/batch-enrich`, which writes taxonomy and classification rows directly under owner credentials, outside every gate `mcp.save_enrichment` enforces; the old runner stays unused until the classifier's pilot, then goes. The classifier uses one instrument per dimension: deterministic code where the evidence is a quotable phrase, a decision model where judgment turns on context. It picks only existing terms and writes one classification per posting through `mcp.save_enrichment` on the action role, or writes nothing and records why. Every label records its instrument, and every model judgment keeps its probabilities against the run's per-posting outcome, so thresholds re-tune from stored evidence without a new model call. The role question reads the description with the title masked: the gap between title and role is what the research measures, and a question that sees the title measures the title. A posting whose role the taxonomy does not cover is deferred, and the minting skills pick it up as unclassified. A classifier cannot reach every skill, so until the measure engine reads lineage and treats an unreachable skill as absent rather than zero, live classifier runs stop at a pilot. Its seniority is its own instrument, distinct from the skills' v9 reading, and recorded by lineage.

Title-stated seniority and classifier-inferred seniority are two different fields, and they are meant to disagree on `Manager`. `open_posting_titles.title_seniority` (000036) is the rank a title states, derived from `title_seniority_seeds` — a vocabulary that counts `manager` as a rank, because employers write it as one. `classifications.seniority` is the rank the classifier infers from the posting description; under the classifier's contract, `Manager` is role-forming, not rank-forming — Product Manager and Account Manager name functions, not levels — so the classifier never infers it as a seniority. The two fields will systematically disagree on Manager titles. That disagreement is signal, not error: it is the gap between what the title claims and what the classifier reads the role to be.

## ATS targets

Six adapters are live: Greenhouse, Lever, Ashby, Workday, Workable, and Gem. Each ATS is a separate file in `apps/tools/internal/ats`; all adapters return `domain.Posting` from `apps/tools/internal/domain`.

| Platform | `board_token` format | API type |
|---|---|---|
| Greenhouse | `<slug>` | GET (Job Board API) |
| Lever | `<slug>` | GET (public postings JSON) |
| Ashby | `<slug>` | GET (public board) |
| Workday | `{host}/{site}` (e.g. `nvidia.wd5.myworkdayjobs.com/NVIDIAExternalCareerSite`) | POST (Workday CXS public `/jobs` endpoint) |
| Workable | `<slug>` (lowercase slug from `apply.workable.com/<slug>`) | GET (public widget endpoint) |
| Gem | `<slug>` (case-sensitive slug from `jobs.gem.com/<slug>`) | GET (public job-board API) |

Greenhouse does not expose structured pay data on the public Job Board API for any board in the current watchlist; compensation appears only in description HTML. Lever is the structured compensation source.

Workday adapter v1 returns the listing-level fields only; per-posting description fetch (the CXS `/job/{id}` endpoint) is deferred to a v1 follow-up. Workday tenants gated behind `wday_vps_cookie` session cookies are unsupported in v1 and surface as fetch errors.

Workable adapter v1 returns the listing-level fields only; the widget endpoint is summary-only and per-posting description fetch (via the `/spi/v3/jobs/{shortcode}` endpoint or HTML scrape of the apply page) is deferred. Workable postings carry NULL `description_text` and will not classify until that follow-up ships.

Gem's single list endpoint returns a bare array with full descriptions. Gem postings classify immediately; no per-posting description fetch is needed.

Four platforms distinguish a job from a job post. Greenhouse, Workday, Workable, and Gem each expose a requisition identifier separate from the posting's own id, so one requisition can carry several postings. Ashby and Lever expose one id, and it is the posting identity the adapter already records — there is no second identity to capture, and synthesizing one would assert a distinction the platform does not make. The split is a property of the platforms, not of the adapters.

Greenhouse exposes two requisition-shaped fields and only one is usable. The internal job id is present on effectively every posting; the field named for requisitions is absent on some boards and filled with prose on others.

Gem boards are Greenhouse-shaped: the payload carries the same internal job id field, and it means the same thing there, the requisition identifier, separate from the posting's own id. The shared shape is platform lineage, not a shared identity space — anything reading that field must still branch on the company's recorded platform rather than sniffing the payload, since the same field name on a different platform carries no guarantee of meaning the same thing.

## The database as AI agent knowledge store

The Postgres DB is also intended to serve as a knowledge store for an AI agent layer. pgvector is load-bearing for this. Semantic search queries are raw SQL — vector ops don't map to ORM query builders.

## Evidence trust tiers

Agent-written records carry signals of three trust tiers. Auditing any agent-written row starts with the tier of each signal.

| Tier | Meaning | Example |
|---|---|---|
| Tool-attested | A tool observed it directly. Ground truth. | `add_company` probe result |
| Deterministic | Computed from agent-supplied evidence. Reproducible — but no more trustworthy than its inputs. | `dedup_candidates`, `detect_ats` verdicts |
| Agent-asserted | The agent says so. | Browser-observed URLs, notes |

Records that mix tiers keep them distinguishable — never collapse a probe result and an agent claim into one field. Unattended runs are audited by walking each signal down this gradient.

## Non-goals (current scope)

- Scheduler (deferred)
- Embedding storage for classification summaries (deferred — pgvector columns not yet added; summary is report-only today; classification provenance schema is live in migration 000001)
- `skills[].requirement` persistence (deferred — writeback ignores the field today)
- Deployment, auth, and multi-tenancy (out of scope — local-first, single operator, one profile per install)
- Text-to-SQL over the schema (rejected — unverifiable numbers, no derivable deep link. The composition grammar is the sanctioned alternative: the model picks typed primitives, the backend computes the aggregate, the model narrates. Handing the model SQL erases the line the whole surface depends on. See [`research/composition-grammar.md`](../../research/composition-grammar.md))
- Fixed view catalog as the deliverable (reframed — the seven scored views survive only as seed compositions over the grammar, not as hand-built screens)
- Compensation and raw geography as answerable dimensions (structurally absent from the grammar vocabulary, not refused per question — the data cannot support them). Curated `market` remains supported.
