...
| Model access | OpenRouter, runtime — interactive surfaces only | First runtime model-API dependency. Serves résumé extraction, widget narration, and chat. Bulk classification uses the direct-MCP Codex skill, not OpenRouter. One key across model families makes composition quality measurable per model. |
...
The interactive surfaces — résumé extraction, widget narration, chat — are the project's first runtime model-API dependency, reached through OpenRouter. A deliberate split, not a drift. Interactive batch classification uses the project-local Codex skill and constrained MCP action, while the Go runner remains an automation path; neither needs an OpenRouter write path. Interactive calls are low-volume, per-person, and the research subject itself — so they earn a runtime API, and one key across model families is what makes composition quality measurable per model. Across both, the model selects and combines primitives; it never computes an aggregate or emits a number. The backend computes; the model narrates.
...
Classification runs on its own cadence. The interactive Codex workflow selects a bounded cohort through MCP, strips per-company boilerplate, and dispatches model workers in waves. Workers validate and save one structured candidate through the action-role-only `mcp.save_enrichment` boundary; a transaction-scoped database advisory lock serializes those writes so cross-table taxonomy ownership remains consistent even when workers are concurrent. The coordinator reconciles persisted classifications between waves and alone maintains the append-only failure history and report. The Go runner, `cmd/batch-enrich`, does not share that boundary. It keeps its own prompt-version lineage and writes taxonomy and classification rows directly under owner credentials, through slug-keyed upserts, so none of what `mcp.save_enrichment` enforces reaches it: the name-collision and retired-slug gates, required provenance, advisory persistence, the serializing lock. It is a second, ungated write route, and stays unused until it is rewritten. A replacement enrichment tool writes through `mcp.save_enrichment`. One that calls the SQL function directly rather than through MCP also skips the Go-side validation, so the function's own checks are its only guard.
