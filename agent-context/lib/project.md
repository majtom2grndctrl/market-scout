...
| Read model | SQL views in numbered migrations | Derived state — cohort membership (open/closed/all), latest snapshot and classification, per-posting lifespan — defined once in SQL rather than per consumer. Openness generalizes to an as-of reconstruction so change-over-time reads the same definition. |
...
| Measure engine | Typed compute over the read model | Turns a validated composition into typed rows plus a cohort-scoped coverage denominator, in the web DB layer over the read-model views. Owns only the arithmetic a view cannot — differencing, normalizing, weekly bucketing, duration. Never hands SQL to the model; a classified grouping returns its denominator inline, so a partial-coverage slice cannot imply full coverage. |
...
That open definition generalizes rather than standing alone. The closed and all cohorts, and openness reconstructed as of a past instant, are all defined against the same open view — so a change-over-time measure and a current count cannot disagree about what "open" means. Classification covers only part of the corpus, and covers each cohort by a different fraction, so any measure grouped or filtered on a classified dimension carries a cohort-scoped coverage denominator: the count it was computed over, against that cohort's total. The denominator travels with the numbers because a chart over a third of the corpus that implied the whole would lie.
