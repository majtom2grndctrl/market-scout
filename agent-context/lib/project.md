...
| Model access | OpenRouter, runtime — chat surface only | First runtime model-API dependency. Bulk classification uses the direct-MCP Codex skill, not OpenRouter. One key across model families makes composition quality measurable per model. |
...
The chat surface is the project's first runtime model-API dependency, reached through OpenRouter. A deliberate split, not a drift. Interactive batch classification uses the project-local Codex skill and constrained MCP action, while the Go runner remains an automation path; neither needs an OpenRouter write path. Interactive analysis is low-volume, per-question, and is the research subject itself — so it earns a runtime API, and one key across model families is what makes composition quality measurable per model. Across both, the model selects and combines primitives; it never computes an aggregate or emits a number. The backend computes; the model narrates.
