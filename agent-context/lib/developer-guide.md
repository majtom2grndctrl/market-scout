...
| `apps/web/tokens/generated/*` | design-token codegen | yes | `pnpm tokens` |

There are no `go:generate` directives in the tree. Both generators run by hand, and each output diff is committed alongside the source change that caused it. `pnpm tokens --check` runs in `apps/web`'s build, so a stale token artifact fails the build rather than shipping.
...
- Missing a design-token step or a utility class? Edit the token module under `apps/web/tokens/`, then regenerate. The CSS and the prop types both come from there.
