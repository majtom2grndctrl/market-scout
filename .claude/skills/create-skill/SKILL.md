...
- `argument-hint` — autocomplete hint like `[file-path]` or `[plan-name]`
- `hooks` — skill-scoped hooks that only fire while the skill is active

**Content principles:**
- Prose follows `agent-context/lib/style-guide.md` — direct and brief, one constraint per bullet.
...
- Sub-agent prompts inline their content; paths drift.
- Cost awareness: skills that spawn agents or run commands respect the cost map (developer-guide §2) — free-tier verification only, live-surface commands stay with the coordinator or the human.
