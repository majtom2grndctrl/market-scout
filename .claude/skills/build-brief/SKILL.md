...
name: build-brief
description: >
  Executes a promoted problem brief end to end. Compact briefs use one
  continuous build with light checkpoints. Resumable briefs add an owner
...
The integrating executor owns `plan.md`, shared contracts, migrations, commits, and final verification. It may delegate bounded implementation slices. Every worker reads the whole brief and relevant context. Never dispatch a task paragraph alone.

## Locate the brief

!`ls agent-context/plans/ready/ agent-context/plans/in-progress/ 2>/dev/null`

A brief's header line starts `Brief ·`. Anything else in these directories — a `/draft-plan` spec, a `/one-pager`, a build-session contract — belongs to another skill.
...
Start from clean, current `main`. Create `feat/<brief-name>` and move the brief from `ready/` to `in-progress/`. Do not commit the move yet.

Read, in order:

1. `agent-context/lib/index.md` and routed docs.
2. The surface guides: `developer-guide.md` and `testing-guide.md` for `tools`; `web-guide.md` and `web-testing-guide.md` for `web`; both sets when the brief spans both.
...
| AC | Proof | Status |
|---|---|---|
| 1 | `go test ./internal/<pkg> -run <Filter>` | achievable as stated |
| 4 | owner, live fetch run | manual |
...
- Named files or package ownership.
- Upstream contracts and downstream consumers.
- Instruction not to edit `plan.md`, run migrations, run `sqlc generate`, or commit.

Run independent workers concurrently when their files and contracts do not overlap. The integrating executor resolves shared seams, owns migration numbering and `sqlc generate`, and applies migrations to the development and test databases. Worktrees share one Postgres cluster; database-backed tests (`go test -tags=integration`, `pnpm test:db`) run on the branch after concurrent edits finish, never from two worktrees at once.

For each task:

1. Read dependent code before editing.
2. Implement and integrate.
3. Run focused tests; confirm filters matched tests — a Go `-run` filter matching nothing prints `ok` with `[no tests to run]`.
4. Update the task row with proof and status.

Free tier only (`developer-guide.md` §2 Cost map). A live fetch run or batch-enrich wave that an AC needs is manual proof, owned by the owner.
...
Run the `/preflight` gates after integration. Then run `/review-panel` and `/fix-findings` as a review → fix → focused retest loop. Repeat only while new concrete findings appear.
...
Update durable `agent-context/lib/` contracts. Move the brief to `agent-context/plans/done/`. Commit the move, plan, and context updates with the final coherent change.
...
Report the branch, landing table, review loop, and outstanding manual checks. Name the two or three choices worth the owner's understanding — idiomatic Go on `tools`, interaction and design-system decisions on `web`.

When the owner says "land the plane": remove session worktrees, then push the branch.
...
- Integrating executor owns seams, migrations, tests, and commits.
