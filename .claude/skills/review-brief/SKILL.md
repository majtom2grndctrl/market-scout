...
name: review-brief
description: >
  Opt-in detail review of a problem brief in `agent-context/plans/drafts/`.
  Three lenses — premise, rows, subtract — each confined to the sections it
  may change: reviewers may add Acceptance rows and research pin rows, and
  nothing else is edited; every finding on Decisions, Problem or Path goes to
  the owner. Never /review-draft-spec on a brief. Run after /validate-plan,
  before promotion, when the stakes warrant an independent read — a
  migration, a write path, an MCP tool shape, a view other consumers read.
  Not a default step.
...
`/review-draft-spec` is built for a spec's anatomy: task paragraphs to fact-check, identifiers to anchor, prose fixes to apply. Run on a brief, its lenses still emit that shape, and applied it does one thing reliably: it appends reviewer prose to Decisions. Every clause a finding adds to a Decision is binding on the executor, so one round turns judgment the owner recorded into mechanism the reviewer prescribed. The brief grows back toward a spec, one applied fix at a time.
...
Argument is a plan folder name; look in `drafts/` first, then `ready/` for an a-la-carte read on a brief already promoted. The line under the title must start `Brief ·`; on a `/draft-plan` spec, stop and point at `/review-draft-spec`.
...
One message, three `Agent` calls on `opus`, read-only, fresh context. Or one agent carrying all three in the order below — if so, the subtract lens answers **before** the agent opens any source file; once it has adjudicated identifiers, the shape question gets a closing paragraph instead of attention.

Every prompt carries: the confirmed absolute paths of the brief and `research.md`, the `agent-context/lib/` docs the header names, the source files Path cites, `agent-context/lib/style-guide.md` to read first, the table above, the finding contract, and where to write its findings file. Report only — no edits, no migrations, no live-surface commands. Read-only MCP `query` against real data is allowed when a premise is about what the data holds.

**Premise lens** — Decisions only. For each bullet, find the claim about the code or schema it rests on and open the symbol. Three verdicts: *holds*, *false*, *unverifiable*. A *false* premise is the highest-value finding this skill produces and it goes straight to the owner; the reviewer does not propose a replacement decision. An `### Agent-facing surface` block gets the same treatment: every name in the example either resolves against the MCP server or a view, or is one the brief adds. Nothing outside Decisions is fact-checked here — Path claims are the executor's.

**Rows lens** — Acceptance and the pin table. Two passes:

- *Ordering.* Attack the Decisions temporally: a fetch that fails after a success, a posting absent from one run and back in the next, zero rows vs no run, a re-run over rows already written, two writers on one row, a migration applied over existing data. Every ordering that is implied and not written becomes a row — `(id, scenario, ordering, expected outcome)` in `research.md`, and an Acceptance row that cites it. A row is the whole output; the ordering is never a sentence added to a Decision.
- *Achievable and proving.* Per Acceptance row: can a test assert this against current source — a unit test with a fake, an integration test against the test database, a Vitest render? Could it pass on a build that leaves the Problem's defect in place? A negative-existence row is a grep gate; say so. Once for the section: is there a Decision with no row that would fail if it were violated, and is there a row no Decision makes necessary?
...
- Restated facts and bare counts in prose; length against the selected mode's target, excluding a required boundary table. Locate any excess by section.
