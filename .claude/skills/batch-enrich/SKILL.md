...
**Reference queries** (latest-snapshot-per-posting via `LATERAL`):

`force=true` (description_text filter only):
```sql
...
`force=false` (also excludes already-classified postings):
```sql
SELECT jp.id AS posting_id,
       jp.company_id,
       s.title,
...
`company_id` and each element of `selected_ids` must be validated as integers before interpolation into this JSON template.
...
Wave size: **10**. For each wave, send 10 Agent tool calls in a **single message** — true parallel dispatch is the point of this skill. Wait for all 10 to return before starting the next wave. Final wave may be smaller than 10. Agents run in parallel waves. Phase B writebacks run serially — one posting at a time, in sequence.
...
**Repair retry loop (per agent in the wave).** After each agent in the wave returns, attempt to parse its output as JSON. If parsing fails:

1. If an agent ID was returned by the Agent tool call: send a `SendMessage` to that agent with the retry directive: "your previous output was invalid JSON, return only the corrected JSON" followed by the prior raw response. If `SendMessage` errors, fall through to step 2.
2. If no agent ID was returned, or `SendMessage` errored: dispatch a fresh Agent call with the same prompt plus the prior raw response appended.

Both paths count toward the **4** total call cap (1 initial + up to 3 retries). The first parseable response wins and proceeds to Phase A validation normally. If all retries are exhausted without a parseable response, mark the posting `json_failed` and log every attempt (see Step 8d). Repair retries reuse the same `PROMPT_VERSION` pinned at invocation start — the repair directive is not a versioned prompt change.
...
The Agent tool call returns both the agent's response and an **agent ID**. Retain this ID per posting — it is the address used to issue repair retries via `SendMessage` (see Step 6) when the agent's response is unparseable JSON.
...
- Every emitted canonical_role — new or existing — must include a non-empty `dimensions` array. An empty array or missing key is a validation failure; the orchestrator skips the posting.
...
Read `PROMPT_VERSION` and `MODEL` from the `classification-pins` block at the top of this file once at invocation start. Apply both uniformly to every posting in the batch. Parse the block as exact `KEY=VALUE` lines, no spaces around `=`; orchestrator greps `^PROMPT_VERSION=` and `^MODEL=` from the block. Validate that both values match `^[A-Za-z0-9._-]+$`; if either fails, abort the skill — do not proceed to writeback with an unvalidated model or version string.

This writeback path uses raw psql rather than the Go query layer — the orchestrator interpolates values into SQL string literals. The validation and escaping rules in §8a are mandatory: agent-emitted strings are untrusted input.
...
**Rule A — Slug validation.** Every slug emitted by the agent (canonical_role slugs, specialization slugs, skill slugs, dimension slugs) must match `^[a-z0-9]+(-[a-z0-9]+)*$` and be at most 64 characters. This rejects leading/trailing dashes and consecutive dashes. A slug that fails either check is a validation failure.

**Rule B — String escaping.** Before interpolating any `name` or `notes` value into a SQL string literal, replace every `'` with `''` (SQL single-quote doubling). Apply to canonical_role names, specialization names, skill names, and classification notes. Slugs already passed Rule A and need no escaping. Seniority is validated against the closed set in §8b step 0 and therefore needs no Rule B escaping. Relies on `standard_conforming_strings = on` (Postgres default since 9.1 — the Docker image used here sets this). If `\` appears in a name or notes value, it is treated as a literal backslash, not an escape character.

**Rule C — Null byte rejection.** Before interpolating any `name` or `notes` value, reject (skip posting, log validation failure) any value containing the null byte (`\x00`). Postgres rejects it at the protocol level; a mid-transaction abort is not a clean skip.

These rules apply to every interpolation in both phases below.

#### 8b. Phase A — Pre-flight (no transaction)

Terminology: **parse failure** = the agent response is not valid JSON (triggers the repair retry loop in Step 6); **validation failure** = the parsed object violates the agent contract (skip posting, log to `failures.jsonl`, no retry).

Retain each agent's raw response string alongside the parsed object through all of Phase A — `failures.jsonl` requires the original text, not a re-serialization of the parsed object.

For each posting, in order:

0. **Validate seniority.** If `classification.seniority` is missing or null, skip the posting, log a validation failure (`posting_id` + `seniority missing`), continue the batch. Also verify `classification.seniority` is one of the allowed values: `intern`, `junior`, `mid`, `senior`, `staff`, `principal`, `lead`, `director`, `unknown`. If the value is present but not in this set, skip the posting, log a validation failure (`posting_id` + `seniority invalid: <value>`), continue.
...
The orchestrator builds the complete SQL script with all validated values already inlined, then pipes it via `<<'SQL'`. Do not use shell variables inside the heredoc — the single-quoted delimiter prevents shell expansion.
...
2. **Upsert canonical_role_dimensions.** For each `(canonical_role_id, dimension_id)` pair from the Phase A mapping:
   ...
   ON CONFLICT (canonical_role_id, dimension_id) DO NOTHING;
   ```
...
3. **Insert classifications row and capture id.** One row per posting per run. Use psql's `\gset` to bind the returned id to a script variable for the join inserts in step 4:
   ...
   For `<notes_or_null>`: if notes is null, missing, or empty after stripping Unicode whitespace (leading and trailing, per Go `strings.TrimSpace`), interpolate the bare keyword `NULL` (no quotes). Otherwise interpolate `'<escaped-notes>'` (with Rule B escaping applied).
...
- `summary` — not persisted yet; pgvector storage is deferred. Field is part of the agent output contract — preserve it in the parsed object and include it in the per-posting report summary.
...
- **Recurring failures.** Read `agent-output/batch-enrich/failures.jsonl` (tolerate a missing or empty file — if absent or empty, render the section as `(none)` and do not error). Group lines by `posting_id` and count the number of **distinct `run_timestamp` values** per posting. Render only postings with a count ≥ 2. For each such posting, include:
  ...
  TODO: add a rotation strategy for `failures.jsonl` — the file is scanned in full every run with no size bound.
...
- **Fail loudly.** Log every parse failure and validation failure with posting id and reason.
