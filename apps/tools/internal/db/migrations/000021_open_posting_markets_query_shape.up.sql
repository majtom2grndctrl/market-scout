-- Keep filtered market reads planner-inlineable without changing the
-- append-only snapshot read model. Local performance check (2026-08-21,
-- 4,595 open postings): after applying this migration in a dev database,
-- record the median of five warm EXPLAIN (ANALYZE, TIMING OFF) runs for a
-- job_posting_id-filtered open_posting_markets query before expanding the
-- dictionary.

CREATE OR REPLACE VIEW open_posting_markets AS
WITH matched_markets AS NOT MATERIALIZED (
    SELECT DISTINCT
