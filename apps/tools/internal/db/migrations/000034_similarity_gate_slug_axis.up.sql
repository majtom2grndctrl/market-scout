-- Narrow the 000033 near-duplicate gate's substitution branch to the slug axis.
--
-- 000033 remapped a proposed new slug onto an existing taxonomy row when EITHER
-- the normalized names matched exactly OR
-- greatest(similarity(slug, slug), similarity(name, name)) cleared 0.95. Its own
-- header called the name axis the weak one and put both knobs in constants at
-- the top of the body specifically so a follow-up could narrow the branch
-- without restructuring anything. This is that follow-up.
--
-- What changes
-- ------------
--   * Substitution fires only on similarity(proposed.slug, existing.slug) at or
--     above c_substitute_at, on the axis named by c_substitute_axis. Exact
--     normalized-name equality no longer substitutes on its own.
--   * An exact-name match that does not clear the slug threshold is demoted to
--     an advisory `similarity_candidates` entry, tagged match:'exact_name' and
--     carrying both axis scores, so the agent still sees the match, sees why it
--     was suggested, and decides for itself.
--
-- Why the name axis had to go
-- ---------------------------
-- `name` is not a reliable identity signal in this database. Measured on the
-- live `skills` table: 110 pairs share an exact normalized name, and 47 of them
-- (42.7%) score below 0.3 on slug similarity -- different concepts carrying the
-- same name string, the same corruption 000030 section 3 repaired by hand for
-- "Cloud Platforms". The worst cases, by how much of the corpus they touch:
--
--   problem-solving (812 uses) / critical-thinking
--       both named "Problem Solving"                             slug sim 0.063
--   llm (410) / embeddings
--       both named "Large Language Models"                        slug sim 0.000
--   observability (276) / instrumentation
--       both named "Observability, Monitoring & Instrumentation"  slug sim 0.000
--   pytorch (164) / tensorflow
--       both named "PyTorch or TensorFlow"                        slug sim 0.000
--
-- Under 000033, a novel skill carrying the name "Large Language Models" was
-- silently remapped onto whichever of llm / embeddings sorted first. Nothing in
-- the payload, the envelope or the write made that visible as a guess. That is
-- the failure this migration removes.
--
-- On the slug axis alone, similarity >= 0.95 over every intra-table pair yields
-- exactly two hits across all three tables --
-- infrastructure-security / security-infrastructure and
-- customer-technical-enablement / technical-customer-enablement -- both genuine
-- word-order restatements. That is the clean zone, and it is now the only zone
-- that substitutes.
--
-- Why demotion rather than deletion
-- ---------------------------------
-- Substituting on a bad name match is strictly worse than minting a duplicate.
-- Minting keeps the correct slug and corrupts only a display string, which is
-- repairable in one pass. Remapping discards the correct concept, keeps the
-- wrong one, and writes a classification link pointing at the wrong taxonomy
-- row -- and the payload that would have carried the correct slug is gone. So
-- the gate now fails toward minting.
--
-- But losing the name signal entirely would be its own regression: it is the
-- only thing that catches the case that motivated 000033 at all.
-- data-orchestration was minted carrying the name "Data Pipeline Architecture"
-- while data-pipeline-architecture already held exactly that name, and the two
-- slugs score 0.150 against each other. Under this migration that case mints
-- and reports an advisory candidate naming the existing row. It costs one round
-- trip instead of returning a silent wrong answer.
--
-- Implementation note: two rankings, not one
-- ------------------------------------------
-- 000033 ranked each novel proposal once and took rn = 1 as the substitution
-- target. That was sound only because the ranking's first key, exact_name DESC,
-- was itself a substitution trigger. It no longer is. An exact-name row scoring
-- 0.063 on slug can outrank a genuine 1.000 slug match, and a single ranking
-- would hand rn = 1 to the row that can no longer substitute while losing the
-- one that can -- a silent loss of the only behaviour being kept. Phase A
-- therefore ranks twice: once over the substitutable rows alone, ordered by
-- axis score, and once over everything reportable, ordered as before for the
-- advisory list. The advisory list still suppresses any proposal the
-- substitution ranking claimed, so the two never both report the same index.
--
-- The axis is a named constant, c_substitute_axis, alongside the three
-- thresholds 000033 already named. There is deliberately no value meaning
-- "either axis" -- that is 000033's behaviour, and it is what this migration
-- removes. The CASE has no ELSE, so an unrecognised value yields NULL, which
-- compares false against the threshold: a typo mints instead of remapping,
-- which is the safe direction.
--
-- pg_trgm's punctuation stripping is unchanged and still applies to the slug
-- axis, but slugs are punctuation-free by construction (c_slug_pattern), so the
-- similarity('C++','C#') = 1.000 hazard 000033 documented no longer reaches the
-- substitution branch at all. It survives only in the advisory `sim` column,
-- where it costs nothing.
--
-- Preserved from 000033 exactly as built
-- --------------------------------------
--   * The within-payload redundancy drop at c_payload_dup_at = 0.85, still on
--     greatest(slug, name) -- inside one payload the name axis is the agent's
--     own text for two entries it just wrote, not a stored column with a
--     decade of drift, and the phase drops a redundant tag rather than
--     redirecting a concept.
--   * The advisory floor at c_advisory_at = 0.60.
--   * Phase ordering: substitution (A) before payload dedup (C), so a remap
--     that collides inside an array is still resolved there.
--   * The dimension merge when C drops an element onto the same slug.
--   * The envelope keys `substitutions`, `dropped` and `similarity_candidates`
--     appearing only when non-empty, so an untouched payload returns exactly
--     what 000031 returned.
--   * The 000026 advisory-lock wrapper and the external
--     mcp.save_enrichment(jsonb, text, text) signature -- CREATE OR REPLACE
--     targets save_enrichment_unlocked only, so the wrapper body, its lock keys
--     and the action-role EXECUTE grant that follows its OID are untouched.
--   * Every pre-existing check, still running on the payload AS SUBMITTED and
--     still before the gate reshapes anything: slug shape and length, the
--     within-array duplicate_slug error, cross-array and cross-table
--     slug_collision, empty/unknown dimensions, and the 000031 retired_slug
--     registry.
--   * The v_has_novel and v_needs_dedup pre-checks, so a payload with nothing
--     to substitute and nothing redundant still pays the pre-000033 cost.
--
-- Cost is unchanged. Phase A scores the same two similarity() calls per
-- (novel proposal x existing row) pair as 000033 did; the axis CASE picks
-- between values already computed. The second row_number() is a window over the
-- rows already materialised, not a second scan of the taxonomy.
