-- Close the similarity gate's mint path on normalized name.
--
-- 000033 gated proposed taxonomy terms against the existing vocabulary on two
-- axes, slug and name, and let either one SUBSTITUTE: a proposal whose name
-- matched an existing row was silently remapped onto that row. Names were
-- corrupted, so that remapped genuinely novel terms onto unrelated slugs -- a
-- new "Large Language Models" skill landing on `llm` or `embeddings` at slug
-- similarity 0.000, writing a wrong link with nothing to show it happened.
-- 000034 demoted the name axis to advisory, choosing to fail toward minting:
-- minting corrupts a display string, remapping discards a concept.
--
-- The cost of that choice is that name-identical, slug-dissimilar terms mint
-- freely. 24 skills and 4 specializations did between 2026-09-18 and today.
--
-- Substitute is not block. Substituting writes the wrong row; blocking writes
-- nothing and returns a structured error naming the collider, so the worker
-- reuses the slug or picks a distinguishing name. Nothing is guessed, and
-- 000034's failure mode is not reintroduced.
--
-- This is affordable now because 000037 brought normalized-name collisions in
-- `skills` and `specializations` to zero. Before it, the rule would have fired
-- against 110 existing pairs and workers would have learned to route around
-- it. `canonical_roles` is still in that state and is therefore excluded.
--
-- No UNIQUE index on normalized name. An index cannot tell minting from reuse,
-- surfaces as an unstructured constraint violation instead of a structured
-- error, and forecloses the rare collision a human decides is legitimate.
--
-- The whole function is re-declared because that is the only way to change a
-- plpgsql body. The only change from 000034 is the `name_collision` block and
-- its three declarations; the advisory-lock wrapper `mcp.save_enrichment` is
-- untouched, so writes stay serialized as 000026 arranged.
