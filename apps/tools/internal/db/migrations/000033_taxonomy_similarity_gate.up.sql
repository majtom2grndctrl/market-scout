-- Near-duplicate suppression inside the enrichment write path.
--
-- 000031 gave save_enrichment a memory for slugs that must never be minted
-- again. This migration gives it a memory for slugs it has ALREADY minted:
-- a proposal that is a restatement of an existing taxonomy row is redirected
-- onto that row instead of creating a second one.
--
-- Why substitution and not rejection
-- ----------------------------------
-- mcp.save_enrichment_unlocked accumulates errors and returns
-- {ok:false, errors:[...]} for the whole call. It is all-or-nothing: one
-- rejected skill slug discards the classification row, the seniority, the
-- roles, the specializations, the notes and every other good skill, and the
-- agent must resubmit the lot. The cheapest exit for a blocked agent is
-- therefore not a better slug -- it is dropping the tag. That is measured
-- behaviour here: when agents could not confirm a match they dropped tags
-- rather than minting, and average skills per posting fell from 8.37 to 4.61.
-- A rejection gate would install that incentive at the tool boundary, where
-- no prompt can argue it out. So this gate never rejects. It remaps, or it
-- advises, or it deterministically drops a redundant entry -- and the write
-- always lands.
--
-- There is also no rejection threshold that would work. Between roughly 0.70
-- and 0.85 the false positives are unfixable by tuning because the
-- interleaving is in the data itself: technical-accounting-manager vs
-- technical-account-manager score 0.833 and are different jobs (finance vs
-- sales); reliability-engineer vs site-reliability-engineer score 0.808 and
-- arguably are the same job.
--
-- No confirm_new escape hatch is added. With substitution there is no wall to
-- escape, and an agent that hits a wall twice just sets the flag.
--
-- What this adds, in order
-- ------------------------
--   A. Substitution. A proposal whose slug does not yet exist in its target
--      table is compared against every existing row in that SAME table, on
--      both slug and name, with pg_trgm. On an exact normalized-name match or
--      a similarity at or above c_substitute_at, the proposed slug is silently
--      remapped onto the existing slug and reported in `substitutions`.
--   B. Advisory candidates. A proposal that still mints, but whose best match
--      lands between c_advisory_at and c_substitute_at, is reported in
--      `similarity_candidates`. Nothing is blocked; the agent gets to learn.
--   C. Within-payload redundancy. Two entries in the same array scoring at or
--      above c_payload_dup_at against each other are one concept twice. One
--      survivor is chosen deterministically (higher existing usage, then
--      shorter slug, then alphabetically) and the rest are dropped and
--      reported in `dropped`.
--
-- A runs before C on purpose: A can itself create a within-payload duplicate
-- by remapping two different proposals onto the same existing slug, and C is
-- what resolves that. C therefore also guarantees each array holds distinct
-- slugs before the write loops run, so a remap-into-collision cannot reach the
-- insert path twice.
--
-- Everything 000012 / 000026 / 000031 already did is preserved: the external
-- mcp.save_enrichment(jsonb,text,text) signature, the 000026 advisory-lock
-- wrapper (untouched -- CREATE OR REPLACE targets save_enrichment_unlocked
-- only, so the wrapper's body, its lock keys and the action-role EXECUTE grant
-- that follows its OID are all unaffected), the cross-table slug_collision
-- checks, the 000031 retired_slug check, and the within-array v_seen
-- duplicate_slug detection. All of those still run, unchanged, on the payload
-- AS SUBMITTED -- the gate only reshapes the payload after every check has
-- passed, so a retired slug is still rejected even when it would have
-- substituted, and a duplicate the agent actually submitted is still an error
-- rather than a silent drop.
--
-- Evidence behind the thresholds (measured on the live tables: 266 canonical
-- roles, 357 specializations, 2261 skills)
-- ----------------------------------------------------------------------
--   * SLUG similarity >= 0.95 over every intra-table pair yields exactly two
--     hits, both genuine word-order restatements
--     (infrastructure-security/security-infrastructure,
--     customer-technical-enablement/technical-customer-enablement). That axis
--     is clean.
--   * NAME similarity at the same threshold is not. 110 skill pairs share an
--     exact normalized name while their slugs are well under 0.95, and close
--     to half of them are different concepts whose `name` column was written
--     wrong at mint time -- the same corruption 000030 section 3 fixed for
--     "Cloud Platforms". javascript carries the name "HTML/CSS"; terraform
--     carries "Infrastructure as Code (Terraform)"; embeddings carries "Large
--     Language Models"; tensorflow carries "PyTorch or TensorFlow";
--     data-analysis carries "Statistical Analysis and Experimental Design".
--     Against each of those, the exact-name branch below substitutes -- and is
--     wrong. It is kept because it is the branch the motivating case needs
--     (data-orchestration was minted with the name "Data Pipeline
--     Architecture" while data-pipeline-architecture already held exactly that
--     name), but the reader should know it is the weak axis, not the strong
--     one. The two knobs are constants at the top of the body so a follow-up
--     migration can narrow the substitute branch to the slug axis alone
--     without restructuring anything.
--   * pg_trgm strips punctuation before trigramming, so similarity('C++','C#')
--     is 1.000. Short punctuation-heavy names are therefore matched by the
--     similarity branch even when they are plainly different. The exact-name
--     branch does NOT have this problem: it compares normalized text, and
--     'c++' <> 'c#'.
--   * The within-payload threshold of 0.85 is the brief's. Both cases that
--     motivated it fall below it and will NOT be caught:
--     similarity('docker','kubernetes-docker') is 0.389 and
--     similarity('cloud-platform-architecture','platform-architecture') is
--     0.786. Catching the first needs word_similarity (a containment metric
--     that also scores aws/aws-lambda at 1.000, which would drop the specific
--     skill in favour of the general one), so it is deliberately not used.
--     0.85 is kept as specified; it is a constant.
--
-- Cost
-- ----
-- Phase A compares each novel slug against every row of its table, so it scans
-- the taxonomy: measured at roughly 20ms per novel slug on 2884 taxonomy rows
-- (a 10-skill payload with 4 novel slugs went from ~3ms to ~87ms). A payload
-- that mints nothing is guarded back to the pre-000033 cost of ~3ms. Since the
-- 000026 wrapper serializes every save globally, the worst case caps enrichment
-- writes near 11/second; model latency dominates a real run by orders of
-- magnitude, so this is not on the critical path today. If it ever is, the fix
-- is GIN trgm indexes on (slug) and (name) for the three taxonomy tables plus a
-- `SET pg_trgm.similarity_threshold` clause on this function so the scan can
-- become an index probe on the `%` operator -- deliberately not done here,
-- because it is six new indexes on the enrichment write path for a cost nothing
-- is currently paying.
