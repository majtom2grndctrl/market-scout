-- Persist the similarity gate's advisory candidates.
--
-- 000033 gave `mcp.save_enrichment` a `similarity_candidates` array: every
-- near match the gate found for a term the worker was about to mint, with its
-- scores, handed back so the worker can reuse an existing slug instead. 000034
-- widened it and 000038 kept it. Nothing has ever written it down. The array
-- is built, returned in the envelope, read once by whoever is holding the
-- session, and lost.
--
-- What that costs is the only number that says whether the gate works. The
-- vocabulary-health report at the end of a batch-enrich run wants to say "N
-- advisories were shown and M were ignored". With no persisted record it can
-- only ask the workers, and on 2026-09-21 two of them filed self-contradictory
-- self-reports in the same run -- one claiming a reuse while having minted,
-- and conceding it only under direct challenge. Every claim about mint
-- discipline, then and since, is unfalsifiable. This table is what makes the
-- next one checkable.
--
-- SHAPE. One row per (proposal x candidate the gate showed for it), not one
-- row per call and not one JSON blob per call. The question the table exists
-- to answer -- how often did a worker mint despite being shown a near match,
-- and how near was it -- is a GROUP BY over (outcome, similarity) on that
-- shape and a jsonb_array_elements gauntlet on any other. A call that produced
-- no near matches writes nothing, which is why this is a separate table rather
-- than columns on `classifications`: the overwhelming majority of calls have
-- nothing to say here.
--
-- TWO SOURCES. Advisory candidates (the gate showed a neighbour and let the
-- worker decide) and substitutions (the gate remapped the proposal onto the
-- neighbour itself, at slug similarity >= 0.95) both land here, tagged by
-- `candidate_source`. They are the same fact -- this proposal had a neighbour
-- -- differing only in who acted on it, and splitting them across two tables
-- would make "what fraction of near matches got reused" a union instead of a
-- filter. Only substitutions can currently produce outcome='reused', which is
-- exactly the contrast the table is for.
--
-- OUTCOME IS OBSERVED, NOT PREDICTED. `outcome` is read off the write that
-- actually happened -- 'minted' iff the proposed slug is in the set of rows
-- the write loops newly inserted -- rather than inferred from which branch of
-- the gate produced the row. Given today's gate the two agree, and if a later
-- change breaks that agreement this column reports it instead of concealing
-- it. That is the whole point: a self-report is what this table replaces.
--
-- BOTH AXES. `slug_similarity` and `name_similarity` are stored separately and
-- `advisory_similarity` is the combined figure the worker actually saw. 000034
-- demoted the name axis precisely because the two disagree -- 47 skill pairs
-- shared an exact name at slug similarity under 0.3 -- so a table that kept
-- only the combined score could not distinguish "the worker ignored a true
-- duplicate" from "the worker correctly ignored a corrupted label". The
-- envelope reports only the greater of the two, so `name_similarity` is
-- recomputed at insert time with the same IMMUTABLE public.similarity call on
-- the same two strings. Widening the returned envelope instead would be a
-- contract change, and the skill's pin is being bumped separately.
--
-- APPEND-ONLY, like every other table this function writes. Rows are inserted
-- and never updated or deleted; a second opinion about the same term is
-- another row, and the history is the sequence. Nothing enforces that with a
-- trigger, for the same reason nothing enforces it on `posting_snapshots` or
-- `classifications`: the write path is one SECURITY DEFINER function, the
-- action role cannot reach the table directly, and the read-only role has
-- SELECT and nothing else.
--
-- REJECTED: logging to a text log instead. The advisory-ignored rate has to be
-- joinable against `classifications`, `skills` and `job_postings` to be worth
-- anything -- "which model ignored advisories most" and "did the terms it
-- minted ever get used again" are both joins -- and a log file is not.
--
-- REJECTED: writing the whole `similarity_candidates` envelope as one jsonb
-- column. It would be a faithful transcript and near-useless to query. The
-- columns below are the transcript; nothing in the envelope is dropped except
-- `path`, which is an index into a payload array that is not stored anywhere.
--
-- The function is re-declared in full because that is the only way to change a
-- plpgsql body. The only change from 000038 is the audit INSERT after the
-- write loops; the gate itself is untouched, and so is the advisory-lock
-- wrapper `mcp.save_enrichment`, so writes stay serialized as 000026 arranged.

-- ---------------------------------------------------------------------------
-- 1. The audit table.
--
-- `classification_id` and `job_posting_id` are both stored. The second is
-- derivable through the first, and it is here anyway: "was this term proposed
-- for this posting before" is a question asked against a posting, and every
-- classification of a posting is a separate row in a table that keeps all of
-- them.
--
-- ON DELETE CASCADE matches the three job_posting_* join tables. An audit row
-- describes a classification that happened; if the classification is gone the
-- row describes nothing.
--
-- numeric(4,3) is the range the gate's own round(x, 3) produces: 0.000 to
-- 1.000. A score outside it is a bug, and the type says so.
-- ---------------------------------------------------------------------------

CREATE TABLE taxonomy_similarity_advisories (
    id                  bigserial    PRIMARY KEY,
    classification_id   bigint       NOT NULL
        REFERENCES classifications (id) ON DELETE CASCADE,
    job_posting_id      bigint       NOT NULL
        REFERENCES job_postings (id) ON DELETE CASCADE,
    -- Which taxonomy table the proposal targeted.
    table_name          text         NOT NULL,
    -- The term as the worker proposed it, before any remap.
    proposed_slug       text         NOT NULL,
    proposed_name       text         NOT NULL,
    -- The existing row the gate offered against it.
    candidate_slug      text         NOT NULL,
    candidate_name      text         NOT NULL,
    -- Report order, 1-based: rank 1 is the nearest match the worker was shown.
    candidate_rank      int          NOT NULL,
    -- 'advisory'     -- shown to the worker, who was free to ignore it.
    -- 'substitution' -- the gate remapped the proposal onto it (000033/000034).
    candidate_source    text         NOT NULL,
    -- Why this candidate surfaced, verbatim from the envelope: 'exact_name' or
    -- 'similarity' for an advisory, 'slug_similarity' for a substitution.
    match_kind          text         NOT NULL,
    slug_similarity     numeric(4,3) NOT NULL,
    name_similarity     numeric(4,3) NOT NULL,
    -- The single score the envelope showed the worker: the greater of the two
    -- axes for an advisory, the substituting axis's score for a substitution.
    advisory_similarity numeric(4,3) NOT NULL,
    -- What the write actually did with the proposed term.
    outcome             text         NOT NULL,
    -- Provenance of the call, copied from the same arguments the
    -- `classifications` row records, so the audit stands alone in a query.
    model               text         NOT NULL,
    prompt_version      text         NOT NULL,
    created_at          timestamptz  NOT NULL DEFAULT now(),
    CONSTRAINT taxonomy_similarity_advisories_table_name_check
        CHECK (table_name IN ('canonical_roles', 'specializations', 'skills')),
    CONSTRAINT taxonomy_similarity_advisories_source_check
        CHECK (candidate_source IN ('advisory', 'substitution')),
    CONSTRAINT taxonomy_similarity_advisories_outcome_check
        CHECK (outcome IN ('minted', 'reused')),
    CONSTRAINT taxonomy_similarity_advisories_rank_check
        CHECK (candidate_rank >= 1)
);

-- The mint-discipline query: filter to advisories, group by outcome, read the
-- similarity distribution. Leading with outcome keeps that scan off the heap.
CREATE INDEX taxonomy_similarity_advisories_outcome_idx
    ON taxonomy_similarity_advisories (outcome, candidate_source, created_at DESC);

-- "Has this term been proposed before, and against what?" -- the per-term
-- history, which is how a repeated near-duplicate gets noticed.
CREATE INDEX taxonomy_similarity_advisories_term_idx
    ON taxonomy_similarity_advisories (table_name, proposed_slug);

-- Reverse lookup from a classification under review to what the gate told the
-- worker while it was being written.
CREATE INDEX taxonomy_similarity_advisories_classification_idx
    ON taxonomy_similarity_advisories (classification_id);

COMMENT ON TABLE taxonomy_similarity_advisories IS
...
DECLARE
    ...
    -- ---- Mint-only name collision (migration 000038) ---------------------
    -- The colliding row's identity and both axis scores, so the error names
    -- the collider instead of leaving the worker to search for it.
    v_collider_slug   text;
    v_collider_name   text;
    -- The normalizer the exact_name axis already uses below. One definition:
    -- a second one here would drift from the axis it is supposed to agree
    -- with, and the disagreement would be invisible.
    v_norm_name       text;
...
BEGIN
    ...
    -- Mint-only name collision (migration 000038): a payload term that is
    -- about to be MINTED into `specializations` or `skills` must not carry the
    -- normalized name of a row that is already there. Reuse of an existing
    -- slug is never blocked -- only minting a second row for a name the
    -- taxonomy already spells.
    --
    ...
    --
    -- `canonical_roles` is deliberately out of scope. 000037 brought
    -- normalized-name collisions in these two tables to zero, which is what
    -- makes the rule affordable; roles still carry theirs, and a rule that
    -- fires against existing corruption is a rule workers learn to route
    -- around. Roles rejoin when their dedup lands.
    --
    ...
    v_idx := 0;
    FOR v_elem IN
        SELECT jsonb_array_elements(coalesce(p_payload -> 'specializations', '[]'::jsonb))
    LOOP
        v_slug := v_elem ->> 'slug';
        v_name := v_elem ->> 'name';
        v_norm_name := lower(btrim(regexp_replace(coalesce(v_name, ''), '\s+', ' ', 'g')));

        IF v_norm_name <> ''
           AND NOT EXISTS (SELECT 1 FROM public.specializations WHERE slug = v_slug) THEN
            SELECT existing.slug, existing.name
              INTO v_collider_slug, v_collider_name
            FROM public.specializations AS existing
            WHERE lower(btrim(regexp_replace(existing.name, '\s+', ' ', 'g'))) = v_norm_name
            ORDER BY public.similarity(existing.slug, v_slug) DESC, existing.id
            LIMIT 1;

            IF v_collider_slug IS NOT NULL THEN
                v_errors := v_errors || jsonb_build_object(
                    'path', format('specializations[%s].name', v_idx),
                    'code', 'name_collision',
                    'table', 'specializations',
                    'proposed_slug', v_slug,
                    'proposed_name', v_name,
                    'existing_slug', v_collider_slug,
                    'existing_name', v_collider_name,
                    'slug_similarity', round(public.similarity(v_slug, v_collider_slug)::numeric, 3),
                    'name_similarity', round(public.similarity(coalesce(v_name, ''), v_collider_name)::numeric, 3),
                    'message', format(
                        'specializations already has %L named %L; minting %L under the same name would split one term across two rows. Reuse %L, or name the new concept so it is distinguishable.',
                        v_collider_slug, v_collider_name, v_slug, v_collider_slug)
                );
            END IF;
        END IF;
        v_idx := v_idx + 1;
    ...
    v_idx := 0;
    FOR v_elem IN
        SELECT jsonb_array_elements(coalesce(p_payload -> 'skills', '[]'::jsonb))
    LOOP
        v_slug := v_elem ->> 'slug';
        v_name := v_elem ->> 'name';
        v_norm_name := lower(btrim(regexp_replace(coalesce(v_name, ''), '\s+', ' ', 'g')));
        ...
        IF v_norm_name <> ''
           AND NOT EXISTS (SELECT 1 FROM public.skills WHERE slug = v_slug) THEN
            SELECT existing.slug, existing.name
              INTO v_collider_slug, v_collider_name
            ...
            IF v_collider_slug IS NOT NULL THEN
                v_errors := v_errors || jsonb_build_object(
                    'path', format('skills[%s].name', v_idx),
                    'code', 'name_collision',
                    'table', 'skills',
                    'proposed_slug', v_slug,
                    'proposed_name', v_name,
                    ...
                    'name_similarity', round(public.similarity(coalesce(v_name, ''), v_collider_name)::numeric, 3),
                    'message', format(
                        'skills already has %L named %L; minting %L under the same name would split one term across two rows. Reuse %L, or name the new concept so it is distinguishable.',
                        v_collider_slug, v_collider_name, v_slug, v_collider_slug)
                );
            END IF;
        END IF;
