-- Reverse 000040 in full.
--
-- The up migration deleted 5 taxonomy rows, moved 90 join rows and unioned two
-- sets of role dimensions, so this file cannot be written from the map alone.
-- 000037's `retired_slug_links` holds every deleted row's id, slug, name and
-- created_at plus every link that moved; this migration's own
-- `retired_role_dimensions` holds every dimension tag that moved. Both record
-- whether the survivor already held the pair, which is what keeps this file
-- from deleting rows that predate 000040.
--
-- Original ids are restored, not regenerated, for 000037's reason: the
-- identity sequences have already advanced past the deleted ids, so
-- reinserting them cannot collide with a row minted since. The sequences are
-- resynchronised afterwards because an explicit-id INSERT does not advance
-- them.
--
-- Order is load-bearing. The gate has to stop blocking on the wider key before
-- the duplicate rows come back, or nothing downstream could mint against them;
-- the restored rows have to exist before their links and dimensions can point
-- at them; and both have to be back before the retired_slugs entries and the
-- archives are removed.

-- ---------------------------------------------------------------------------
-- 1. Restore the gate to its 000039 definition: `canonical_roles` out of
-- scope again, and the comparison key back to collapse-and-trim, so
-- name-identical roles and internal-spacing variants mint freely as they did
-- before 000040.
--
-- The function below is 000039's definition verbatim.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Put back the name 000040 repaired.
-- ---------------------------------------------------------------------------

UPDATE canonical_roles SET name = 'Business Operations Manager' WHERE slug = 'business-operations';

-- ---------------------------------------------------------------------------
-- 3. Recreate the deleted taxonomy rows at their original ids, then
...
INSERT INTO canonical_roles (id, slug, name, created_at)
...
WHERE l.retired_by_migration = '000040_roles_name_gate_and_spacing'
...
SELECT setval('canonical_roles_id_seq', (SELECT max(id) FROM canonical_roles));
SELECT setval('skills_id_seq',          (SELECT max(id) FROM skills));

-- ---------------------------------------------------------------------------
-- 4. Put the links and the dimension tags back on the restored rows.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Remove the survivor links and dimension tags 000040 created.
--
-- `collided = false` is what that column is for, in both archives: a pair the
-- up migration's ON CONFLICT DO NOTHING skipped because the survivor already
-- held it predates 000040 and must survive this file. Only the rows 000040
-- actually inserted are removed.
-- ---------------------------------------------------------------------------

DELETE FROM job_posting_roles j
USING retired_slug_links l, canonical_roles s
WHERE l.retired_by_migration = '000040_roles_name_gate_and_spacing'
  AND l.table_name = 'canonical_roles'
  AND l.classification_id IS NOT NULL
...
USING retired_role_dimensions a, retired_slug_links l, canonical_roles s, role_dimensions d
WHERE a.retired_by_migration = '000040_roles_name_gate_and_spacing'
  AND a.collided = false
  AND l.retired_by_migration = a.retired_by_migration
  AND l.table_name = 'canonical_roles'
  AND l.retired_slug = a.retired_slug
  AND s.slug = l.survivor_slug
  AND d.slug = a.dimension_slug
  AND crd.canonical_role_id = s.id
  AND crd.dimension_id = d.id;

-- ---------------------------------------------------------------------------
-- 6. Unregister the retirements and drop this migration's archives.
--
-- `retired_slug_links` is 000037's table and stays; only this migration's rows
-- leave it. `retired_role_dimensions` is this migration's own and goes.
-- ---------------------------------------------------------------------------

DELETE FROM retired_slugs
WHERE retired_by_migration = '000040_roles_name_gate_and_spacing';

DELETE FROM retired_slug_links
WHERE retired_by_migration = '000040_roles_name_gate_and_spacing';

DROP TABLE retired_role_dimensions;
