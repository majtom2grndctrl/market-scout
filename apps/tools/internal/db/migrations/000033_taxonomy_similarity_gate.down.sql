-- Reverse 000033: restore the 000031 body of mcp.save_enrichment_unlocked
-- verbatim -- the retired-slug registry check, with no near-duplicate gate.
--
...
--
-- Nothing else to undo: 000033 created no table, no index and no extension.
-- pg_trgm predates it and stays. The reverted body simply stops consulting the
-- taxonomy for near matches, so the next payload that would have been remapped
-- mints its own row again.
