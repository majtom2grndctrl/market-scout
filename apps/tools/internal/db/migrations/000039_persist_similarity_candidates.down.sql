-- Reverse 000039 in full: stop auditing advisories, and drop the record.
--
-- The table is dropped rather than kept. It holds nothing that predates 000039
-- and nothing any other object references, so keeping it would leave a table
-- that no longer fills and whose last row silently marks the rollback. A
-- rollback that wants the history should dump it first; this file's job is to
-- return the schema to its 000038 shape.
--
-- Order is load-bearing: the function has to stop referencing the table before
-- the table can go, or the next save_enrichment call between the two
-- statements raises on a missing relation. CREATE OR REPLACE first, DROP
-- second.
--
-- The function below is 000038's definition verbatim.
...
-- ---------------------------------------------------------------------------
-- The table is now unreferenced.
-- ---------------------------------------------------------------------------

DROP TABLE taxonomy_similarity_advisories;
