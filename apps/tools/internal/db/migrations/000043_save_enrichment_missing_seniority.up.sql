-- Reject a missing or null classification.seniority with a structured error.
--
-- mcp.save_enrichment_unlocked checked seniority with
-- NOT (v_seniority = ANY (c_seniorities)). When the payload omits the field,
-- or sends it as JSON null, v_seniority is NULL. NULL = ANY (...) is NULL, NOT
-- NULL is NULL, and IF treats NULL as false, so the check never fired. The
-- call went on to the INSERT and failed on the classifications.seniority NOT
-- NULL constraint, which reached the caller as a raw database exception
-- instead of an ok:false envelope.
--
-- v9 of the classifier contract makes abstaining common and asks workers to
-- send 'unknown' when they abstain. A worker that omits the field instead now
-- gets missing_seniority on classification.seniority, in the same errors[]
-- batch as every other validation failure, with a message that names the fix.
-- The code is the one the Go validator (internal/enrich/classify) already
-- emits for the same condition, and blank counts as missing there too, so the
-- two layers now agree. The MCP tool's Go layer catches this first; this check
-- is for callers that reach the function without it.
--
-- Nothing else changes. The accepted set is still exactly the
-- classifications_seniority_check set, and a non-null value outside it still
-- reports invalid_seniority with the same message. The advisory-lock wrapper
-- mcp.save_enrichment is untouched, and so is every check after this one.
--
-- The other classification field has no such hole. classification.notes is
-- nullable, and the write already folds null and blank to NULL.
--
-- The whole function is re-declared because that is the only way to change a
-- plpgsql body. It is 000040's definition with the one seniority block
-- replaced.
...
BEGIN
    ...
    -- Seniority is required and must be in the closed set. Both are re-checked
    -- here instead of relying on the table's NOT NULL and CHECK constraints to
    -- RAISE, which would surface to the caller as db_error rather than a
    -- structured error.
    --
    -- Missing is tested first and separately (migration 000043). The
    -- membership test cannot see a NULL: NULL = ANY (...) is NULL, and IF
    -- treats NULL as false. Blank counts as missing, as it does in
    -- internal/enrich/classify/validate.go, which emits the same code.
    IF v_seniority IS NULL OR v_seniority !~ '\S' THEN
        v_errors := v_errors || jsonb_build_object(
            'path', 'classification.seniority',
            'code', 'missing_seniority',
            'message', format(
                'seniority is required, even when you abstain: send %L rather than omitting the field. Valid values: %s',
                'unknown', array_to_string(c_seniorities, ', '))
        );
    ELSIF NOT (v_seniority = ANY (c_seniorities)) THEN
        v_errors := v_errors || jsonb_build_object(
            'path', 'classification.seniority',
