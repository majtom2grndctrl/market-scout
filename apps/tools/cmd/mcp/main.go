...
func newMCPServer(pools dbPools) *server.MCPServer {
	...
	addCompanyTool := mcp.NewTool(
		...
		mcp.WithString("ats", mcp.Required(), mcp.Description("ATS key: greenhouse, lever, ashby, workday, workable, or gem.")),
		mcp.WithString("board_token", mcp.Required(), mcp.Description("ATS board token. Workday is {host}/{site}; Workable is the account slug; Lever and Gem tokens are case-sensitive.")),
